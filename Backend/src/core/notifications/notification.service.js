...
/** Roles that own a notification inbox. Must stay in step with the `ownerType`
 *  enum on the notification model — a role missing here cannot read its own
 *  inbox even though rows exist for it. */
const OWNER_TYPES_BY_ROLE = new Set([
    'USER',
    'RESTAURANT',
    'DELIVERY_PARTNER',
    'SERVICE_PROVIDER',
...
const normalizeOwnerType = (role) => {
    ...
    return OWNER_TYPES_BY_ROLE.has(normalized) ? normalized : null;
};

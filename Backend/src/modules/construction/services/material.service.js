...
export const createMaterialRequest = async (customerId, data) => {
  ...
  // adminNote is internal, so it is not part of what the customer gets back.
  const { adminNote, ...forCustomer } = request.toObject();
  return forCustomer;
};
...
export const updateMaterialRequestStatus = async (id, { status, adminNote }, reqUser = null) => {
  ...
  await audit('material_request', 'material_request.status_changed', {
    entityId: request._id,
    before,
    after: { status },
    performedBy: performer,

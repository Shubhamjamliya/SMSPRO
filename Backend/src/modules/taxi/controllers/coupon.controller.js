...
export const previewUserCoupon = asyncHandler(async (req, res) => {
    const preview = await couponService.previewUserCoupon(req.user?.userId, req.body);
    return sendResponse(res, 200, 'Coupon applied preview', { preview });
});

export const listAvailableUserCoupons = asyncHandler(async (req, res) => {
    const data = await couponService.listAvailableUserCoupons(req.user?.userId, req.query);
    return sendResponse(res, 200, 'Available coupons fetched successfully', data);
});

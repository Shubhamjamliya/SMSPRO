...
const returnItemSchema = new mongoose.Schema(
  {
    ...
    /** Variant-safe order-line identity; '' on returns created before the item-level rollout. */
    lineKey: { type: String, trim: true, default: '' },
    /** Legacy itemId retained so historical lookups keep resolving after the key change. */
    legacyItemId: { type: String, trim: true, default: '' },
    /** Order-time category snapshot copied from the order line (audit trail). */
    categoryId: { type: String, trim: true, default: '' },
    headerId: { type: String, trim: true, default: '' },
    /** Item expiry that was in force when this return was accepted. */
    returnEligibleUntilSnapshot: { type: Date, default: null },
...
const sellerReturnSchema = new mongoose.Schema(
  {
    ...
    finance: {
      ...
      /** Legacy seller-side pickup fee debit. Always 0 for returns after the admin-funded rollout. */
      pickupFeeDebited: { type: Number, min: 0, default: 0 },
      /** 'ADMIN' for new returns; '' (or absent) on historical seller-funded returns. */
      pickupFeeFundedBy: { type: String, enum: ['', 'ADMIN', 'SELLER'], default: '' },

...
const quickProductSchema = new mongoose.Schema({
  ...
  salePrice: { type: Number, default: 0 },
  // Per-product flat packing charge (not per qty / not per variant line)
  ...
  clientRequestId: { type: String, default: '', trim: true, index: true },
...
quickProductSchema.index({ sellerId: 1, headerId: 1, approvalStatus: 1, isActive: 1 });
quickProductSchema.index({ sellerId: 1, categoryId: 1, approvalStatus: 1, isActive: 1 });
quickProductSchema.index(

...
import { getActiveFeeSettings, resolveReturnWindowHoursFromProducts } from '../admin/services/billing.service.js';
...
import { syncOrderReturnMirrors } from './returnMirror.service.js';
import {
...
import {
  ...
  enrichReturnItemsFromOrderItems,
...
/**
 * Order-wide window used ONLY by legacy lines that carry no returnPolicySnapshot.
 * Orders created after the item-level rollout skip the category lookup entirely.
 */
const resolveLegacyReturnWindowHours = async (order, feeSettings) => {
  const quickItems = getQuickItemsFromOrder(order);
  const needsLegacyResolution = quickItems.some(
    (item) => !(Number(item?.returnPolicySnapshot?.returnWindowDays) > 0),
  );
  if (!needsLegacyResolution) {
    return Number(feeSettings?.returnWindowHours) || 72;
  }
  return resolveReturnWindowHoursFromProducts(quickItems, feeSettings);
};
...
const loadReturnEligibleOrder = async ({ orderId, userId }) => {
  ...
  if (!deliveredAt) {
    throw new ValidationError(
      'Delivery time is not recorded for this order, so returns cannot be processed.',
      'DELIVERY_TIMESTAMP_MISSING',
    );
  }

  // Legacy order-wide window — only consumed by items that have no policy snapshot.
  const legacyWindowHours = await resolveLegacyReturnWindowHours(order, feeSettings);
  const existingReturns = await SellerReturn.find({ orderId: order.orderId })
    .select('returnStatus returnItems cumulativeReturnItems')
    .lean();
  const priorReturnedMap = buildPriorReturnedQuantityMap(existingReturns);
  const eligibility = buildOrderReturnEligibility({
    order,
    sellerOrders,
    feeSettings,
    legacyWindowHours,
    priorReturnedMap,
    returnDocs: existingReturns,
    itemStatusMap: buildItemReturnStatusMap(existingReturns),
  });

  // Item-level authority: the order is only closed when NO line is still returnable.
  if (!eligibility.anyItemReturnable) {
    if (eligibility.returnAlreadyUsed) {
      throw new ValidationError(
        'A return has already been submitted for this order. Only one return is allowed per order.',
        'RETURN_ALREADY_USED',
      );
    }
    const expiredOnly = eligibility.items.length
      && eligibility.items.every((row) => row.ineligibleReason === 'ALREADY_RETURNED');
    if (expiredOnly) {
      throw new ValidationError(
        'All items on this order have already been returned.',
        'ALREADY_RETURNED',
      );
    }
    const widestDays = eligibility.returnWindowDays;
    throw new ValidationError(
      `Return window has expired. Returns were available for up to ${widestDays} day${widestDays === 1 ? '' : 's'} after delivery.`,
  ...
  return {
    ...
    deliveredAt,
    legacyWindowHours,
    returnWindowHours: legacyWindowHours,
    priorReturnedMap,
    eligibility,
  };
};
...
/**
 * Recompute the denormalized per-item return state on the parent QC order.
 *
 * Fully idempotent: values are recomputed from the SellerReturn documents rather
 * than incremented, so repeated calls (retries, socket replays) cannot drift.
 */
export const syncOrderItemReturnState = async (orderDoc) => {
  if (!orderDoc || !isQuickCommerceOrderType(orderDoc.orderType)) return orderDoc;

  const returns = await SellerReturn.find({ orderId: orderDoc.orderId })
    .select('returnStatus returnItems cumulativeReturnItems')
    .lean();
  if (!returns.length) return orderDoc;

  const settledQtyMap = buildPriorReturnedQuantityMap(returns);
  const activeStatusByKey = buildItemReturnStatusMap(returns);

  (orderDoc.items || []).forEach((item) => {
    if (String(item.type || '') !== 'quick') return;
    const aliases = buildOrderItemKeyAliases(item);
    const matchedQty = aliases
      .map((alias) => settledQtyMap.get(alias))
      .find((value) => value !== undefined);
    const matchedStatus = aliases
      .map((alias) => activeStatusByKey.get(alias))
      .find(Boolean);

    item.returnedQuantity = Math.min(Number(item.quantity || 0), Number(matchedQty || 0));
    item.itemReturnStatus = matchedStatus || (item.returnedQuantity > 0 ? 'refunded' : 'none');
  });
...
export const createQuickCommerceReturnRequest = async ({
  ...
  const { order, priorReturnedMap, eligibility } = await loadReturnEligibleOrder({ orderId, userId });
  ...
  // Server-authoritative: eligibility comes from the order snapshot, never the request body.
  const itemLookup = buildOrderItemLookup(quickItems, eligibility.items);
  const normalizedItems = normalizeReturnRequestItems(items, quickItems, priorReturnedMap, itemLookup);

  const requestedBySeller = new Map();

  normalizedItems.forEach((requestedItem) => {
    const key = String(requestedItem?.itemId || '').trim();
    const matchedItem = itemLookup.get(key)?.item;
    if (!matchedItem) {
  ...
  for (const [sellerId, sellerRequestedItems] of requestedBySeller.entries()) {
    ...
    if (existing && TERMINAL_RETURN_STATUSES.has(existing.returnStatus)) {
      if (existing.returnStatus === RETURN_STATUSES.CANCELLED) {
    ...
    try {
      ...
      // Concurrent double-submit: the {sellerId, orderId} unique index wins the race.
      // Return the winner instead of surfacing a 500 duplicate-key error.
      if (error?.code === 11000) {
        const winner = await SellerReturn.findOne({ sellerId, orderId: order.orderId }).lean();
        if (winner) {
          logger.warn(
            `[QCReturn] Duplicate return submit ignored for order ${order.orderId} seller ${sellerId}`,
          );
          createdReturns.push(serializeReturnForCustomer(winner));
          continue;
        }
      }
      throw error;
  ...
  try {
    ...
    if (parent) {
      ...
      await syncOrderItemReturnState(parent);
      await parent.save({ validateBeforeSave: false });
      await syncOrderReturnMirrors({ orderId: parent.orderId });
  ...
  logger.info(
    `[QCReturn] Return created for order ${order.orderId} by user ${userId}: ` +
      `${normalizedItems.length} line(s), method=${method}`,
  );
...
export const resolveReturnEligibilityForOrder = async (order) => {
  ...
  const [legacyWindowHours, existingReturns] = await Promise.all([
    resolveLegacyReturnWindowHours(order, feeSettings),
    SellerReturn.find({ orderId: order.orderId })
      .select('returnStatus returnItems cumulativeReturnItems')
      .lean(),
  ]);

  return buildOrderReturnEligibility({
    order,
    sellerOrders,
    feeSettings,
    legacyWindowHours,
...
export const getQuickCommerceReturnStatus = async ({ orderId, userId }) => {
  ...
  // Flat fields kept for legacy clients; the per-item array is exposed once, nested.
  const { items: _perItemEligibility, ...flatEligibility } = returnEligibility;

  return {
    orderId: order.orderId,
    parentReturnStatus: order.returnStatus || '',
    returns: returns.map((doc) => {
      const serialized = serializeReturnForCustomer(doc);
      return {
        ...serialized,
        returnItems: enrichReturnItemsFromOrderItems(serialized.returnItems, order.items),
      };
    }),
    ...flatEligibility,
    returnEligibility,
...
export const cancelQuickCommerceReturnRequest = async ({
  ...
  await syncOrderReturnMirrors({ orderId: order.orderId });
...
export const recordSellerReturnDecision = async ({
  ...
  await returnDoc.save();
  await syncOrderReturnMirrors({ orderId: returnDoc.orderId });
...
const enrichAdminReturnRows = async (items = []) => {
  ...
  const parentOrders = orderIds.length
    ? await QuickOrder.find({ orderId: { $in: orderIds } })
        ...
        .select('orderId pricing deliveryAddress customer userId items')
        .lean()
    : [];

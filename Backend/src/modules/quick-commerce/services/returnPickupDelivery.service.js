...
const creditReturnPickupRiderEarning = async (returnDoc, deliveryPartnerId) => {
  ...
  await recordAdminReturnPickupExpense(returnDoc, amount, feeMetadata);
};

/**
 * Fund the rider's return pickup fee from the platform, not the seller.
 *
 * Idempotent on (admin, return_pickup_fee, returnId) so a retried or replayed
 * pickup completion can never post the expense twice.
 */
const recordAdminReturnPickupExpense = async (returnDoc, amount, feeMetadata) => {
  const returnId = String(returnDoc._id);
  ...
  try {
    ...
    if (existingExpense) {
      logger.info(`[ReturnFinance] Admin return pickup expense already posted for return ${returnId}`);
      return;
    }
    ...
    await debitWallet({
      ...
      entityId: 'platform',
      amount,
      description: `Return pickup fee (order ${returnDoc.orderId || returnId})`,
      category: 'return_pickup_fee',
      module: 'quick',
      metadata: { ...feeMetadata, referenceId: `return_pickup_fee:${returnId}`, fundedBy: 'ADMIN' },
    });
    ...
    await SellerReturn.updateOne(
      ...
      {
        $set: {
          'finance.pickupFeeFundedBy': 'ADMIN',
          'finance.pickupFeeAdminExpense': Number(amount),
        },
      },
    );

    logger.info(
      `[ReturnFinance] Admin funded return pickup fee ${amount} for return ${returnId} ` +
        `(order ${returnDoc.orderId}, rider ${feeMetadata.riderId})`,
    );
  } catch (error) {
    // The rider has already been paid; surface the gap instead of failing the trip.
    logger.error(
      `[ReturnFinance] Admin return pickup expense failed for return ${returnId}: ${error?.message || error}`,
    );
  }
};
...
export const completeReturnPickup = async (returnId, deliveryPartnerId, body = {}) => {
  ...
  // Restore inventory when returned items are back with seller
  try {
    if (!returnDoc.inventoryRestored) {
      const { restoreQuickOrderItemsStock } = await import('../utils/stock.helpers.js');
      await restoreQuickOrderItemsStock(
        (returnDoc.returnItems || returnDoc.items || []).map((item) => ({
          productId: item.productId || item.itemId,
          quantity: Number(item.returnedQty || item.quantity || 0),
          variantName: item.variantName || item.notes || item.variantId || '',
          variantKey: item.variantId || item.variantKey || '',
    ...
    logger.warn(`Return stock restore failed for ${returnDoc._id}: ${error?.message || error}`);
  }

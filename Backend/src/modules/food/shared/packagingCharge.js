 ...
 * before pricing runs (see enforceMinimumFoodItemPrices in order.service.js).
 */
 ...
 * Quick-commerce lines are ignored — they carry their own packing fee.
 */
export function computeFoodPackagingFee({ items = [], config = null } = {}) {
    const packagingConfig = config || normalizePackagingConfig(null);
    const foodItems = (Array.isArray(items) ? items : []).filter(
        (item) => item?.type === 'food'
    );

    if (!packagingConfig.isEnabled || foodItems.length === 0) {

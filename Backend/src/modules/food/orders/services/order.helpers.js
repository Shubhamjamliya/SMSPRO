...
import { resolveFoodRiderEarning } from './deliveryFeeCalculation.service.js';
...
export function buildDeliverySocketPayload(orderDoc, restaurantDoc = null) {
  ...
  const payload = {
    ...
    riderBaseDeliveryFee: Number(order?.pricing?.riderBaseDeliveryFee || 0),
    deliveryExtraFee: Number(order?.pricing?.deliveryExtraFee || 0),
    riderEarning: resolveFoodRiderEarning(order?.pricing || {}) || order?.riderEarning || 0,
    earnings: resolveFoodRiderEarning(order?.pricing || {}) || order?.riderEarning || order?.pricing?.riderBaseDeliveryFee || 0,

...
// treats `$ne` as `$not` and REJECTS the whole index spec, so the index is never
// built and the uniqueness declared below silently does not exist. These four
// indexes are what stop a restaurant or rider holding two active subscriptions
// at once, so losing them means double billing. `{ $type: 'objectId' }` is the
// accepted way to say "this reference is present".

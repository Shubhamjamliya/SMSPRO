...
import {
    ...
    uploadCurrentUserIdentityDocument,
...
export const uploadCurrentUserIdentityDocumentController = async (req, res, next) => {
    try {
        const userId = req.user?.userId;
        const documentType = req.body?.documentType || req.query?.documentType;
        const result = await uploadCurrentUserIdentityDocument(userId, req.file, documentType);
        return sendResponse(res, 200, 'Document uploaded successfully', result);
    } catch (error) {
        next(error);
    }
};

    ...
    for (const food of foods) {
        ...
        byCategory.get(groupKey).items.push({
            ...
            packagingCharge: {
                ...
                amount: Math.max(0, Number(food.packagingCharge?.amount) || 0)
            },
...
export async function invalidatePublicRestaurantMenuCache() {
    try {
        ...
        // Menu/category state is denormalised into several cached surfaces: the menu
        // endpoint, the addons endpoint, restaurant detail, the public categories list
        // and unified search. Clearing only menu+categories (as this did) left deleted
        // or deactivated items visible in search and on the detail page until their TTL
        // lapsed. INVALIDATION_GROUPS.MENU keeps that fan-out in one place.
        await invalidateCache(INVALIDATION_GROUPS.MENU);
    } catch (error) {

...
async function main() {
  ...
  const pickupFundedBy = String(returnDoc.finance?.pickupFeeFundedBy || '');

  if (pickupFundedBy === 'ADMIN' || (pickupDebited === 0 && resolvedPickupCharge > 0)) {
    // Current rule: the return pickup fee is an admin expense, never a seller debit.
    if (pickupFeeTxns.length > 0) {
      fail('SellerLedger', `Seller was debited ₹${pickupFeeTxns.reduce((s, t) => s + Math.abs(num(t.amount)), 0)} for an admin-funded pickup fee`);
    } else {
      pass('SellerLedger', 'No seller pickup fee debit (admin-funded)');
    }
  } else if (pickupDebited > 0) {
    // Legacy return recorded before the admin-funded rollout.
    warn('SellerLedger', 'Legacy seller-funded pickup fee (pre-rollout return)');
  ...
  // --- Admin ledger (return pickup fee funded by the platform) ---
  if (resolvedPickupCharge > 0) {
    const adminExpenses = await Transaction.find({
      entityType: 'admin',
      type: 'debit',
    ...
    if (adminExpenses.length === 0) {
      if (pickupDebited > 0) {
        warn('AdminLedger', 'No admin expense (legacy seller-funded return)');
      } else {
        fail('AdminLedger', 'No admin return pickup expense recorded');
      }
    } else if (adminExpenses.length > 1) {
      fail('AdminLedger', `Duplicate admin expenses: ${adminExpenses.length}`);
    } else if (!eq(adminExpenses[0].amount, resolvedPickupCharge)) {
      fail('AdminLedger', `Admin expense ₹${adminExpenses[0].amount} != pickup ₹${resolvedPickupCharge}`);
    } else {
      pass('AdminLedger', `Single admin expense ₹${adminExpenses[0].amount} (funded by ADMIN)`);
    }
  }

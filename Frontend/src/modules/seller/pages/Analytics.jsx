...
const Analytics = () => {
  ...
  const stats = [
    ...
    {
      label: "Return Rate",
      value: statsData?.overview?.returnRate || "0%",
      trend: `${statsData?.returns?.totalReturns ?? 0} returns`,
  ...
  const returnsOverview = statsData?.returns || null;
  ...
  const handleDownloadReport = () => {
    ...
    try {
      ...
      if (trend.length) {
        ...
        lines.push("Period,Sales,Returns,Traffic");
        trend.forEach((d) => {
          lines.push(
            `${escapeCsv(d.name)},${escapeCsv(d.sales)},${escapeCsv(d.returns ?? 0)},${escapeCsv(d.traffic)}`,
          );
        });
        lines.push("");
      }

      const returnsSummary = statsData?.returns;
      if (returnsSummary) {
        lines.push("Returns & Refunds");
        lines.push("Metric,Value");
        lines.push(`Total returns,${escapeCsv(returnsSummary.totalReturns ?? 0)}`);
        lines.push(`Awaiting approval,${escapeCsv(returnsSummary.pendingApproval ?? 0)}`);
        lines.push(`In progress,${escapeCsv(returnsSummary.activeReturns ?? 0)}`);
        lines.push(`Completed,${escapeCsv(returnsSummary.completedReturns ?? 0)}`);
        lines.push(`Rejected,${escapeCsv(returnsSummary.rejectedReturns ?? 0)}`);
        lines.push(`Refunded to customers,${escapeCsv(returnsSummary.refundedToCustomers ?? 0)}`);
        lines.push(`Deducted from earnings,${escapeCsv(returnsSummary.deductedFromEarnings ?? 0)}`);
        lines.push(`Return rate %,${escapeCsv(returnsSummary.returnRate ?? 0)}`);
        lines.push("");
      }
  ...
  return (
    <div className="space-y-6 sm:space-y-8 pb-20 sm:pb-16">
      ...
      {returnsOverview?.totalReturns > 0 &&
      (activeTab === "Overview" || activeTab === "Sales") ? (
        <BlurFade delay={0.35}>
          <Card className="border-none shadow-xl shadow-slate-200/50 rounded-3xl p-6 bg-white">
            <div className="mb-5">
              <h3 className="text-lg font-black text-slate-900">
                Returns &amp; Refunds
              </h3>
              <p className="text-xs text-slate-600 font-bold uppercase tracking-widest">
                What came back and what it cost you
              </p>
            </div>
            <div className="grid grid-cols-2 gap-4 md:grid-cols-3 lg:grid-cols-6">
              {[
                { label: "Total returns", value: returnsOverview.totalReturns, tone: "text-slate-900" },
                { label: "Awaiting approval", value: returnsOverview.pendingApproval, tone: "text-amber-600" },
                { label: "In progress", value: returnsOverview.activeReturns, tone: "text-blue-600" },
                { label: "Completed", value: returnsOverview.completedReturns, tone: "text-emerald-600" },
                {
                  label: "Refunded to customers",
                  value: `₹${Number(returnsOverview.refundedToCustomers || 0).toLocaleString("en-IN")}`,
                  tone: "text-slate-900",
                },
                {
                  label: "Deducted from earnings",
                  value: `₹${Number(returnsOverview.deductedFromEarnings || 0).toLocaleString("en-IN")}`,
                  tone: "text-rose-600",
                },
              ].map((item) => (
                <div
                  ...
                  <p className={cn("mt-1 text-lg font-black", item.tone)}>
                    {item.value}
                  </p>
                </div>
              ))}
          ...
          <Card className="border-none shadow-xl shadow-slate-200/50 rounded-3xl p-6 bg-white overflow-hidden group h-full">
            ...
            <div className="mt-8 grid grid-cols-3 gap-4 border-t border-slate-50 pt-8">
              <div className="text-center">
                <div className="h-10 w-10 bg-primary/5 text-primary rounded-xl flex items-center justify-center mx-auto mb-2">

...
const RETURN_TAB = "Return Pickups";

const ACTIVE_RETURN_PICKUP_STATUSES = new Set([
  "return_approved",
...
const DeliveryTracking = () => {
  ...
  const fetchDeliveries = async () => {
    try {
      ...
      const [response, returnsResponse] = await Promise.all([
        sellerApi.getOrders(),
        sellerApi.getReturns().catch(() => null),
      ]);
      ...
      const returnList = Array.isArray(returnsResponse?.data?.result?.items)
        ? returnsResponse.data.result.items
        : [];

      setReturnPickups(
        returnList
          .filter((row) => row.returnStatus && row.returnStatus !== "return_requested")
          .map((row) => {
            const partner = row.deliveryPartner;
            const requestedAt = row.returnRequestedAt || row.updatedAt;
            return {
              id: row.id || row.returnId,
              orderId: row.orderId,
              isReturn: true,
              returnStatus: row.returnStatus,
              status: getReturnStatusLabel(row.returnStatus),
              refundAmount: Number(row.returnRefundAmount || 0),
              pickupCharge: Number(row.pickupCharge || row.returnPickupCharge || 0),
              itemCount: Array.isArray(row.returnItems) ? row.returnItems.length : 0,
              deliveryBoy: partner
                ? {
              ...
              location: row.dispatch?.completedAt
                ? `Picked up at ${new Date(row.dispatch.completedAt).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}`
                : "Awaiting pickup",
              ...
              address: row.address
                ? `${row.address.address || ""}, ${row.address.city || ""}`.trim()
                : "",
              addressCoords: row.address?.location || null,
            };
          }),
      );
    } catch (error) {
  ...
  const tabs = ["Active", "Completed", RETURN_TAB, "All"];

  const filteredDeliveries = useMemo(() => {
    const source = activeTab === RETURN_TAB ? returnPickups : deliveries;
    const result = source.filter((dlv) => {
      const matchesSearch =
        (dlv.orderId || "").toLowerCase().includes(searchTerm.toLowerCase()) ||
        dlv.deliveryBoy.name.toLowerCase().includes(searchTerm.toLowerCase());

      if (activeTab === RETURN_TAB) return matchesSearch;
    ...
    return result;
  }, [deliveries, returnPickups, searchTerm, activeTab, page, pageSize]);
  ...
  const stats = useMemo(
    () => [
      ...
      },
      {
        ...
        value: returnPickups.filter((r) =>
          ACTIVE_RETURN_PICKUP_STATUSES.has(r.returnStatus),
        ).length,
    ...
    [deliveries, returnPickups],
  );

  const getStatusVariant = (dlv) => {
    if (dlv.isReturn) return getReturnStatusVariant(dlv.returnStatus);
    switch (dlv.status) {
      case "On the Way":
        return "info";
      case "Picked Up":
        ...
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-slate-600 text-base mt-0.5 font-medium">
              Monitor active deliveries, return pickups and assigned delivery partners.
            </p>
          </div>
        </div>
      </BlurFade>
                    ...
                    <motion.div
                      ...
                      <div className="flex flex-col lg:flex-row items-stretch gap-3 sm:gap-1">
                        ...
                        <div className="lg:w-1/3 p-3 sm:p-5 bg-slate-50/50 rounded-lg border border-transparent group-hover:bg-primary/[0.02] group-hover:border-primary/5 transition-all min-w-0">
                          <div className="flex items-center gap-3 sm:gap-4">
                            ...
                            <div className="flex-1 min-w-0">
                              <p
                                className={cn(
                                  "text-[9px] sm:text-[10px] font-black uppercase tracking-[0.2em] mb-0.5",
                                  dlv.isReturn ? "text-rose-600" : "text-primary",
                                )}>
                                {dlv.isReturn ? "Return Pickup Rider" : "Delivery Partner"}
                              </p>
                          ...
                          <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-2 sm:gap-4 mb-3 sm:mb-4">
                            <div className="min-w-0">
                              <div className="flex flex-wrap items-center gap-2 mb-1 sm:mb-1.5">
                                ...
                                <Badge
                                  variant={getStatusVariant(dlv)}
                                  className="text-[8px] sm:text-[9px] font-black px-2 py-0.5 rounded-full uppercase tracking-widest shrink-0"
                                >
                                  {dlv.status}
                                </Badge>
                                {dlv.isReturn && (
                                  <span className="inline-flex items-center gap-1 text-[8px] sm:text-[9px] font-black px-2 py-0.5 rounded-full uppercase tracking-widest bg-rose-50 text-rose-600 shrink-0">
                            ...
                            <div className="sm:text-right shrink-0">
                              <p className="text-[9px] sm:text-xs font-black text-slate-600 uppercase tracking-widest">
                                {dlv.isReturn ? "Return Requested" : "Order Date & Time"}
                              </p>
                          ...
                          <div className="bg-slate-50/50 p-3 sm:p-4 rounded-lg border border-slate-100/50 min-w-0">
                            <div className="flex items-center justify-between gap-2 mb-1.5 sm:mb-2">
                              <p className="text-[9px] sm:text-xs font-black text-slate-600 uppercase tracking-widest flex items-center gap-2">
                                ...
                                {dlv.isReturn ? "Pickup Address" : "Customer Address"}
                              </p>
                              {dlv.addressCoords &&
                        ...
                        <div className="lg:w-16 flex items-center justify-center p-2 sm:p-3 shrink-0">
                          <div
                            className={cn(
                              "h-10 w-10 lg:h-full lg:w-full rounded-lg lg:rounded-r-lg lg:rounded-l-none flex items-center justify-center text-white shadow-xl",
                              dlv.isReturn
                                ? "bg-rose-500 shadow-rose-500/20"
                                : "bg-primary shadow-primary/20",
                            )}>
                            {dlv.isReturn ? (
                              <HiOutlineArrowUturnLeft className="h-5 w-5 group-hover:scale-125 transition-transform" />
                            ) : (
                ...
                {filteredDeliveries.length === 0 && (
                  <div className="flex flex-col items-center justify-center py-20 bg-slate-50/50 rounded-lg border border-dashed border-slate-200">
                    ...
                    <h3 className="text-base font-black text-slate-900">
                      {activeTab === RETURN_TAB
                        ? "No return pickups in progress"

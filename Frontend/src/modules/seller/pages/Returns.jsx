...
const Returns = () => {
    ...
    const fetchReturns = async (options = {}) => {
        ...
        try {
            ...
            if (!silent) showToast("Failed to fetch return requests", "error");
            return [];
        } finally {
    ...
    useEffect(() => {
        ...
        const intervalId = window.setInterval(async () => {
            const items = await fetchReturns({ silent: true });
                                                    ...
                                                    <p className="text-xs text-slate-500 mt-1 line-clamp-2">
                                                        {(ret.returnItems || [])
                                                            .map((item) => {
                                                                ...
                                                                const qty = Number(
                                                                    item.returnedQty ?? item.quantity ?? 0,
                                                                );
                                ...
                                <div className="space-y-1">
                                    ...
                                    <div className="rounded-2xl border border-slate-100 bg-slate-50 p-3 space-y-1 text-xs text-slate-700">
                                        ...
                                        <p>
                                            ...
                                            {Number(selectedReturn.finance?.pickupFeeDebited || 0) === 0 && (

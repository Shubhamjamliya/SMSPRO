...
const getVariantUnitPrice = (variant, product) => {
    ...
    const prodSale = Number(product?.salePrice || 0);
    if (prodSale > 0) return prodSale;
...
const getVariantPriceLabel = (variant, product) => {
    const mrp = Number(variant?.price || 0) > 0
        ? Number(variant.price)
        : Number(product?.price || 0);
    const sale = Number(variant?.salePrice || 0) > 0
        ? Number(variant.salePrice)
        : (Number(product?.salePrice || 0) > 0 ? Number(product.salePrice) : 0);

    if (sale > 0 && mrp > 0 && sale < mrp) {
        return { primary: formatMoney(sale), secondary: formatMoney(mrp) };
    }
    if (sale > 0) return { primary: formatMoney(sale), secondary: null };
    if (mrp > 0) return { primary: formatMoney(mrp), secondary: null };
    return { primary: formatMoney(getVariantUnitPrice(variant, product)), secondary: null };
};
...
const getProductPriceLabel = (product) => {
    ...
    if (sale > 0) return { primary: formatMoney(sale), secondary: null };
    return { primary: formatMoney(mrp), secondary: null };
};
...
const getProductStockValuation = (product) => {
    ...
    if (variants.length > 0) {
        return variants.reduce(
            (sum, v) => sum + Math.max(0, Number(v.stock) || 0) * getVariantUnitPrice(v, product),
            0,
        );
    }
    const unit = Number(product.salePrice || 0) > 0
        ? Number(product.salePrice)
        : Number(product.price || 0);
    return getProductTotalStock(product) * unit;
};
...
const StockManagement = () => {
    ...
    const stats = useMemo(() => [
        { label: 'Total Inventory', value: inventory.reduce((acc, item) => acc + item.stock, 0), icon: HiOutlineCube, color: 'text-primary', bg: 'bg-primary/5', status: 'All' },
        ...
        { label: 'Stock Valuation', value: `₹${inventory.reduce((acc, item) => acc + getProductStockValuation(item), 0).toLocaleString()}`, icon: HiOutlineArrowsUpDown, color: 'text-emerald-600', bg: 'bg-emerald-50', status: 'In Stock' }
    ], [inventory]);
    ...
    const openAdjustModal = (item) => {
        ...
        if (item.variants && item.variants.length > 0) {
            setSelectedVariantId(item.variants[0].id || item.variants[0]._id);
        } else {
                                                ...
                                                <div className="flex flex-col gap-1.5 mt-2">
                                                    {item.variants && item.variants.length > 0 ? (
                                                        item.variants.map((v, idx) => {
                                                            const priceLabel = getVariantPriceLabel(v, item);
                                                            return (
                                                            <div key={v.id || v._id || idx} className="flex items-center justify-between gap-2 text-[10px] bg-slate-50 px-2 py-1 rounded">
                                                                <span className="font-bold text-slate-600 truncate mr-1">{v.name}</span>
                                                                <div className="flex items-center gap-2 shrink-0">
                                                                    <span className="font-black text-slate-900">
                                                                        {priceLabel.primary}
                                                                        {priceLabel.secondary ? (
                                                                            <span className="ml-1 font-semibold text-slate-400 line-through">{priceLabel.secondary}</span>
                                                                        ) : null}
                                                                    </span>
                                                                    <span className={cn("font-black", Number(v.stock) <= item.threshold ? "text-rose-600" : "text-emerald-600")}>
                                                                        {v.stock} u
                                                                    </span>
                                                                </div>
                                                            </div>
                                                    ...
                                                    (() => {
                                                        ...
                                                        return (
                                                            <p className="text-xs font-bold text-slate-900 mt-2">
                                                                {priceLabel.primary}
                                                                {priceLabel.secondary ? (
                                                                    <span className="ml-1.5 text-[10px] font-semibold text-slate-400 line-through">{priceLabel.secondary}</span>
                                                                ) : null}
                                                            </p>
                                                        );
                                                    })()
                                                    ...
                                                    .map((item) => (
                                                        ...
                                                        >
                                                            ...
                                                            <div className="flex flex-col gap-1.5 w-full">
                                                                {item.variants && item.variants.length > 0 ? (
                                                                    item.variants.map((v, idx) => (
                                                                        <div key={v.id || v._id || idx} className="flex items-center gap-6 text-[11px] w-fit">
                                                                            <span className="font-bold text-slate-600 truncate min-w-[80px]">{v.name}</span>
                                                                            <span className={cn("font-black shrink-0", Number(v.stock) <= item.threshold ? "text-rose-600" : "text-slate-900")}>
                                                                                {v.stock} units
                                                                            </span>
                                                                        </div>
                                                                    ))
                                                                ...
                                                                {item.variants && item.variants.length > 0 ? (
                                                                    <div className="flex flex-col gap-1.5">
                                                                        {item.variants.map((v, idx) => {
                                                                            const priceLabel = getVariantPriceLabel(v, item);
                                                                            return (
                                                                                <div key={v.id || v._id || idx} className="text-[11px] leading-tight">
                                                                                    <p className="font-bold text-slate-500 truncate max-w-[120px]">{v.name}</p>
                                                                                    <p className="font-black text-slate-900">
                                                                                        {priceLabel.primary}
                                                                                        {priceLabel.secondary ? (
                                                                                            <span className="ml-1.5 text-[10px] font-semibold text-slate-400 line-through">
                                                                                                {priceLabel.secondary}
                                                                                            </span>
                                                                                        ) : null}
                                                                                    </p>
                                    ...
                                    <div className="flex items-center gap-5">
                                        <div className={cn(
                                            ...
                                            log.type === 'Restock' ? "bg-emerald-50 text-emerald-600" :
                                                log.type === 'Sale' ? "bg-primary/5 text-primary" : "bg-rose-50 text-rose-600"
                                        )}>
                                            ...
                                            <div className="flex items-center gap-2">
                                                ...
                                                <Badge className={cn(
                                                    ...
                                                    log.type === 'Restock' ? "bg-emerald-100 text-emerald-700" :
                                                        log.type === 'Sale' ? "bg-primary/10 text-primary" : "bg-rose-100 text-rose-700"
                                                )}>
                            ...
                            <div className="p-8 space-y-6">
                                <div className="p-4 rounded-2xl bg-slate-50 border border-slate-100 flex items-center gap-4">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-[10px] font-bold text-slate-600 mt-0.5">CURRENT STOCK: <span className="text-slate-900 font-black">
                                            {selectedVariantId
                                                ? selectedItem.variants?.find(v => (v.id || v._id) === selectedVariantId)?.stock || 0
                                                ...
                                                {selectedItem.variants.map(v => {
                                                    ...
                                                    return (
                                                        <button
                                                            ...
                                                            className={cn(
                                                                "p-2 rounded-xl text-xs font-bold border transition-all text-left",
                                                        ...
                                                        >
                                                            ...
                                                            <div className="text-[10px] opacity-80 mt-0.5">{v.stock} in stock</div>
                                                        </button>
                                                    );
                                                })}
                                            </div>

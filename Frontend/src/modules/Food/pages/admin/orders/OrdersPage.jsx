...
export default function OrdersPage({ statusKey = "all" }) {
  ...
  const normalizedOrders = useMemo(() => {
    ...
    return safeOrders.filter(Boolean).map((order) => {
      ...
      const riderBaseDeliveryFee = Number(pricing.riderBaseDeliveryFee || 0)
      const deliveryExtraFee = Number(pricing.deliveryExtraFee || 0)
      const riderEarning = Number(order.riderEarning || 0)
      const deliveryDistanceKm = pricing.deliveryDistanceKm ?? null
      ...
      // Fallback must match the server formula (order.service.js computedTotal),
      // which includes packaging and the delivery-speed surcharge.
      const computedTotal =
        subtotal + deliveryFee + deliverySpeedFee + packagingFee + platformFee + taxAmount - discountAmount
      ...
      const items = Array.isArray(order.items)
        ? order.items.map((item) => ({
            ...
            price: item.price || 0,
            // Kept so the fare breakdown can settle food and quick lines
            // separately on mixed orders, and show per-item packaging.
            type: item.type || "food",
            foodPackagingCharge: Number(item.foodPackagingCharge || 0),
          }))
        : []

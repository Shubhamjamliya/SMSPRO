...
import { getQuantityLimitMessage, getQuantityLimits } from "@food/utils/orderQuantity"
...
function RestaurantDetailsContent() {
  ...
  // Restaurant add-ons and the current sheet's selection, keyed by addon id.
  // How many to add from the item sheet. Starts at the item's minimum (1 by
  // default) — the sheet used to show the cart quantity, which is 0 for an item
  // that isn't in the cart yet.
  ...
  useEffect(() => {
    ...
    // Add-ons are chosen per line, so every time a new item sheet opens the
    // previous selection is cleared.
    setSelectedAddons({})
    setSheetQuantity(getQuantityLimits(selectedItem).min)
  }, [selectedItem])

  // Restaurant add-ons (admin-approved) offered alongside any item on the menu.
  useEffect(() => {
    const restaurantIdForAddons =
      restaurant?.restaurantId || restaurant?._id || restaurant?.id || ""
    if (!restaurantIdForAddons) return undefined

    let cancelled = false
    const fetchAddons = async () => {
      try {
        setLoadingAddons(true)
        const response = await restaurantAPI.getAddonsByRestaurantId(String(restaurantIdForAddons))
        if (cancelled) return
        const list = response?.data?.data?.addons || response?.data?.addons || []
        setRestaurantAddons(Array.isArray(list) ? list : [])
      } catch (error) {
  ...
  /**
   * Selected add-ons. The API mapper keeps only id + quantity, but name/price
   * ride along so a guest cart (no server round-trip yet) can still render and
   * total the line correctly until the customer signs in.
   */
  const buildAddonPayload = useCallback(
    () =>
      Object.entries(selectedAddons)
        ...
        .map(([addonId, quantity]) => {
          const addon = restaurantAddons.find(
            (entry) => String(entry.id || entry._id) === String(addonId),
          )
          return {
            addonId,
    ...
    [selectedAddons, restaurantAddons],
  )

  const selectedAddonsTotal = useMemo(
    () =>
      restaurantAddons.reduce(
        (sum, addon) =>
          sum + Number(addon.price || 0) * Number(selectedAddons[String(addon.id || addon._id)] || 0),
        0,
      ),
    [restaurantAddons, selectedAddons],
  )

  // Helper function to update item quantity in both local state and cart
  const updateItemQuantity = useCallback(async (item, newQuantity, event = null, preferredVariant = null, options = {}) => {
    const { skipMismatchPrompt = false, addons = null, forceAdd = false } = options || {}
    ...
    // Per-item order quantity limits. A first add jumps straight to the minimum
    // (rasgulla min 4 → "Add" puts 4 in), stepping below it removes the line, and
    // stepping past the cap is refused. The server re-checks all of this.
    const quantityLimits = getQuantityLimits(item)
    if (newQuantity > 0) {
      const currentQuantity = Number(quantitiesRef.current[getLineItemIdForDish(item, preferredVariant || getDefaultFoodVariant(item))]) || 0
      if (newQuantity > quantityLimits.max) {
        toast.error(`You can order at most ${quantityLimits.max} of ${item?.name || "this item"}`)
        return
      }
      if (newQuantity < quantityLimits.min) {
        // Below the minimum: coming from nothing means "add the minimum";
        // stepping down from the minimum means "remove".
        newQuantity = currentQuantity > 0 ? 0 : quantityLimits.min
      }
    }
    ...
    // Optimistic local quantity update (instant UI). `forceAdd` means "add this
    // many more" (the sheet's Add item button), everything else sets an absolute
    // quantity (the card's +/- steppers).
    setQuantities((prev) => {
      const next = { ...prev }
      if (newQuantity <= 0) delete next[lineItemId]
      else next[lineItemId] = forceAdd ? previousQty + newQuantity : newQuantity
      quantitiesRef.current = next
      return next
    })
    ...
    try {
      const cartItem = {
        ...
        preparationTime: item.preparationTime,
        minOrderQuantity: quantityLimits.min,
        maxOrderQuantity: quantityLimits.hasMax ? quantityLimits.max : 0,
        // Ids + quantities only; the server resolves names and prices.
      ...
      if (newQuantity <= 0) {
        ...
        // A line with add-ons is its own cart line, so an add-on selection always
        // goes through addToCart — the server merges identical selections.
        const hasAddonSelection = Array.isArray(addons) && addons.length > 0
        const existingCartItem = forceAdd || hasAddonSelection ? null : getCartItem(lineItemId)
        if (existingCartItem) {
          const productInfo = {
            id: lineItemId,
            name: item.name,
  ...
  return (
    ...
    >
              ...
              <div className="flex items-center gap-2 flex-1 max-w-md">
                <div className="relative flex-1">
                  ...
                  <input
                    ...
                    className="w-full pl-10 pr-10 py-2.5 rounded-full border border-gray-200 dark:border-gray-800 shadow-sm bg-white dark:bg-[#1a1a1a] text-sm dark:text-white focus:outline-none focus:ring-2 focus:ring-[#FF6A00] focus:border-transparent"
                    autoFocus
                  />
                  {searchQuery && (
      ...
      <div className="bg-[#F8F9FA] dark:bg-[#0a0a0a] mt-0 relative z-10 px-4 pt-4 min-h-[50vh]">
          ...
          <div className="bg-white dark:bg-[#1a1a1a] rounded-[28px] shadow-[0_10px_40px_rgb(0,0,0,0.04)] border border-gray-100 dark:border-gray-800 p-5 space-y-4">
            ...
            <div className="flex justify-between items-start">
              ...
              <div className="flex flex-col items-end gap-1">
                <div className="bg-[#FF6A00] text-white px-2 py-1 rounded-lg flex items-center gap-1 font-bold text-sm shadow-sm">
            ...
            <div className="flex items-start gap-4">
              <div className="flex items-start gap-2 text-sm font-medium text-gray-700 dark:text-gray-300 flex-1">
            ...
            <div className="flex items-center justify-between gap-4">
              ...
              <div className="flex-shrink-0">
                <Badge className={`${isRestaurantOffline ? "bg-rose-100 text-rose-600 border-rose-200" : "bg-[#FF6A00] text-white border-transparent"} px-4 py-2 rounded-2xl text-xs font-bold shadow-sm whitespace-nowrap`}>
                ...
                <div className="flex items-center gap-2 w-max">
                  <button
                    ...
                    className={`flex items-center gap-2 whitespace-nowrap rounded-full border px-3 py-1.5 text-sm font-semibold transition-colors ${selectedMenuCategory === "all"
                        ? "border-[#FF6A00] bg-red-50 text-[#FF6A00]"
      ...
      {!showFilterSheet && !showMenuSheet && !showMenuOptionsSheet && (
        <div className={`fixed right-4 z-[60] transition-all duration-300 ${cart.length > 0 ? 'bottom-[150px] md:bottom-[100px]' : 'bottom-[80px] md:bottom-8'}`}>
          ...
          >
            <Utensils className="h-5 w-5 text-[#FF6A00] dark:text-white group-hover:rotate-12 transition-transform" />
              ...
              <>
                ...
                >
                  ...
                  <div className="border-t border-gray-200 dark:border-gray-800 px-4 py-4 bg-white dark:bg-[#1a1a1a]">
                    <Button
                      className="w-full bg-[#1a1a1a] dark:bg-[#FF6A00] hover:bg-[#FF6A00] dark:hover:bg-[#C83C00] text-white border-0 flex items-center justify-center gap-2 py-6 rounded-xl font-bold transition-all shadow-lg"
                      onClick={() => setShowMenuSheet(false)}
                    >
                      <X className="h-5 w-5" />
                      Close
              ...
              <>
                ...
                >
                    ...
                    <div className="space-y-2">
                      ...
                      <button
                        ...
                        className={`flex items-center gap-2 px-4 py-2.5 rounded-lg border-2 transition-all w-full ${filters.highlyReordered
                          ? "border-[#FF6A00] dark:border-[#FF6A00] bg-red-50 dark:bg-[#FF6A00]/20 text-[#FF6A00] dark:text-[#FF6A00]"
                          : "border-gray-200 dark:border-gray-700 bg-white dark:bg-[#2a2a2a] text-gray-700 dark:text-gray-300 hover:border-gray-300 dark:hover:border-gray-600"
                          }`}
                      >
                  ...
                  <div className="border-t border-gray-200 dark:border-gray-800 px-4 py-3 flex items-center justify-between bg-white dark:bg-[#1a1a1a]">
                    ...
                    <Button
                      className="bg-[#FF6A00] hover:bg-[#C83C00] text-white px-6 py-2.5 rounded-lg font-bold"
                      onClick={() => setShowFilterSheet(false)}
                    >
                    ...
                    {restaurant?.outlets && Array.isArray(restaurant.outlets) && restaurant.outlets.length > 0 ? (
                      <div className="space-y-2">
                        {restaurant.outlets.map((outlet) => (
                          ...
                          >
                            {outlet?.isNearest && (
                              <div className="flex items-center gap-1.5 mb-2 px-2 py-1 bg-red-50 dark:bg-[#FF6A00]/20 rounded-md">
                                <Zap className="h-3.5 w-3.5 text-[#FF6A00] dark:text-[#FF6A00] fill-[#FF6A00] dark:fill-[#FF6A00]" />
                                <span className="text-xs font-semibold text-[#FF6A00] dark:text-[#FF6A00]">
              ...
              <>
                ...
                >
                  ...
                  <div className="border-t border-gray-200 dark:border-gray-800 px-4 py-4">
                    <Button
                      className="w-full bg-[#FF6A00] hover:bg-[#C83C00] text-white py-3 rounded-lg font-bold"
                      onClick={() => {
                        setShowManageCollections(false)
                      }}
                    >
              ...
              <>
                ...
                >
                  ...
                  <div className="flex-1 overflow-y-auto px-4 py-4">
                    ...
                    {/* Restaurant add-ons — priced per unit of this item by the server */}
                    {loadingAddons ? (
                      <p className="mb-4 text-xs text-gray-400">Loading add-ons…</p>
                      ...
                      <div className="mb-4">
                        ...
                        <p className="mb-2 text-[11px] text-gray-500 dark:text-gray-400">
                          Charged for each unit of this item
                        </p>
                        <div className="space-y-2">
                          {restaurantAddons.map((addon) => {
                            const addonId = String(addon.id || addon._id)
                            const quantity = Number(selectedAddons[addonId] || 0)
                            return (
                              <div
                                key={addonId}
                              ...
                              >
                                <div className="flex min-w-0 items-center gap-2">
                                  <div
                                    className={`flex h-3.5 w-3.5 shrink-0 items-center justify-center rounded-[3px] border ${addon.foodType === "Non-Veg" ? "border-red-600" : "border-green-600"}`}
                                  >
                                    <div
                                      className={`h-1.5 w-1.5 rounded-full ${addon.foodType === "Non-Veg" ? "bg-red-600" : "bg-green-600"}`}
                                    />
                                  </div>
                                ...
                                {quantity > 0 ? (
                                  <div className="flex shrink-0 items-center gap-2 rounded-lg border border-[#FF6A00] px-1.5 py-0.5">
                                    <button
                                      type="button"
                                      aria-label={`Remove one ${addon.name}`}
                                      className="px-1 text-[#FF6A00]"
                                      onClick={() =>
                                        setSelectedAddons((prev) => {
                                          const next = { ...prev }
                                          if (quantity <= 1) delete next[addonId]
                                    ...
                                    <span className="min-w-[1rem] text-center text-xs font-bold tabular-nums text-[#FF6A00]">
                                      {quantity}
                                    </span>
                                    <button
                                      type="button"
                                      ...
                                      className="px-1 text-[#FF6A00] disabled:opacity-40"
                                      onClick={() =>
                                        setSelectedAddons((prev) => ({
                                          ...prev,
                                          [addonId]: Math.min(10, quantity + 1),
                                        }))
                                      }
                                    >
                                  ...
                                  <button
                                    type="button"
                                    className="shrink-0 rounded-lg border border-[#FF6A00] px-3 py-1 text-xs font-bold text-[#FF6A00]"
                                    onClick={() =>
                  ...
                  <div className="border-t border-gray-200 dark:border-gray-800 px-3 sm:px-4 py-3 sm:py-4 bg-white dark:bg-[#1a1a1a]">
                    {getQuantityLimitMessage(selectedItem, selectedItem?.name || "This item") ? (
                      <p className="mb-2 text-[11px] font-semibold text-[#FF6A00]">
                        {getQuantityLimitMessage(selectedItem, selectedItem?.name || "This item")}
                      </p>
                    ) : null}
                      ...
                      <div className={`flex items-center gap-1.5 sm:gap-3 border-2 rounded-lg px-2 sm:px-3 h-[44px] flex-shrink-0 bg-white dark:bg-[#2a2a2a] ${shouldShowGrayscale
                        ...
                        <button
                          onClick={() => {
                            if (!shouldShowGrayscale) {
                              setSheetQuantity((prev) =>
                                Math.max(getQuantityLimits(selectedItem).min, prev - 1),
                              )
                            }
                          }}
                          disabled={
                            sheetQuantity <= getQuantityLimits(selectedItem).min || shouldShowGrayscale
                          }
                        ...
                        <button
                          onClick={() => {
                            if (!shouldShowGrayscale) {
                              setSheetQuantity((prev) =>
                                Math.min(getQuantityLimits(selectedItem).max, prev + 1),
                              )
                            }
                          }}
                          disabled={
                            sheetQuantity >= getQuantityLimits(selectedItem).max || shouldShowGrayscale
                          }
                          className={shouldShowGrayscale
                            ? 'text-gray-300 dark:text-gray-600 cursor-not-allowed'
                        ...
                        onClick={(e) => {
                          if (!shouldShowGrayscale) {
                            updateItemQuantity(
                              ...
                              { addons: buildAddonPayload(), forceAdd: true },
                            )
                      ...
                      >
                        ...
                        <div className="flex items-center gap-1 min-w-0 overflow-hidden">
                          ...
                          <span className="text-sm sm:text-base font-bold truncate">
                            {(() => {
                              // Unit price shown on the button includes the chosen
                              // add-ons, matching how the server prices the line.
                              const variant = getVariantForDish(selectedItem, selectedVariantId)
                              const unitPrice =
                                Number(variant?.price ?? selectedItem.price ?? 0) + selectedAddonsTotal
                              const lineTotal = unitPrice * sheetQuantity
                              return hasFoodVariants(selectedItem)
                                ? `${getFoodVariantLabel(variant || { name: "Default" })} · ${RUPEE_SYMBOL}${Math.round(lineTotal)}`
                                : `${RUPEE_SYMBOL}${Math.round(lineTotal)}`
                            })()}
                          </span>
                        </div>
                      </Button>

...
import { startRing, stopRing, hasNativeShell } from '@core/native/nativeBridge';
...
const isTaxiOrPorterOffer = (orderData = {}) => {
  const moduleKey = String(orderData?.module || orderData?.jobType || '').toLowerCase();
  return (
    moduleKey === 'taxi' ||
    moduleKey === 'ride' ||
    moduleKey === 'porter' ||
    moduleKey === 'parcel' ||
    Boolean(orderData?.rideId) ||
    Boolean(orderData?.tripId)
  );
};

const buildDeliveryOrderNotification = (orderData = {}) => {
  const orderId = orderData.orderId || orderData.orderMongoId || orderData.rideId || orderData.id || 'New';
  ...
  const isTaxi = isTaxiOrPorterOffer(orderData) && (
    String(orderData?.module || orderData?.jobType || '').toLowerCase() === 'taxi' ||
    String(orderData?.module || orderData?.jobType || '').toLowerCase() === 'ride' ||
    Boolean(orderData?.rideId)
  );
  ...
  return {
    title: orderData.tripType === 'return_pickup' || orderData.documentType === 'seller_return'
      ...
      : isTaxi
        ? `New taxi ride #${orderId}`
        : `New order #${orderId}`,
    body: itemCount > 0
      ? `${itemCount} item${itemCount === 1 ? '' : 's'} - ₹${total.toFixed(2)}`
      : isTaxi
        ? 'A new taxi ride is available to accept'
...
const isActionableDeliveryOffer = (orderData = {}) => {
  if (isTaxiOrPorterOffer(orderData)) {
    const dispatchStatus = String(
      orderData?.dispatch?.status || orderData?.dispatchStatus || '',
    ).trim().toLowerCase();
    if (['accepted', 'completed', 'cancelled'].includes(dispatchStatus)) {
      return false;
    }
    const status = String(
      orderData?.orderStatus || orderData?.status || '',
    ).trim().toLowerCase();
    if (status && /cancel|claimed|accepted|arriving|arrived|in_progress|completed|awaiting_payment/.test(status)) {
      return false;
    }
    return Boolean(
      orderData?.rideId ||
...
const triggerWebViewNativeNotification = async (orderData = {}) => {
  ...
  const offerId =
    orderData?.orderId ||
    orderData?.order_id ||
    orderData?.rideNumber ||
    orderData?.rideId ||
    orderData?.orderMongoId ||
    orderData?.id ||
    '';

  if (isTaxi && hasNativeShell()) {
    try {
      await startRing({
        title: 'New taxi ride',
        body: `Ride #${offerId}`.trim(),
        orderId: offerId,
        order_id: offerId,
        orderMongoId: orderData?.orderMongoId || orderData?.rideId || '',
        rideId: orderData?.rideId || orderData?.orderMongoId || '',
        module: 'taxi',
        type: 'NEW_RIDE',
        ring: true,
      ...
      // Fall through to Flutter handlers / web audio.
    }
  }

  const bridgePayload = {
    title: isTaxi ? 'New taxi ride' : 'New delivery order',
    body: `${isTaxi ? 'Ride' : 'Order'} #${offerId}`.trim(),
    orderId: offerId,
    orderMongoId: orderData?.orderMongoId || orderData?.order_mongo_id || orderData?.rideId || '',
    rideId: orderData?.rideId || '',
    module: isTaxi ? 'taxi' : (orderData?.module || 'food'),
    targetUrl: '/delivery',
  };
  ...
  const recoverDeliveryState = useCallback(async () => {
    ...
    try {
      ...
      const recoverableOrder = availableOrders.find((order) => {
        ...
        const actionableStatuses = isReturnPickup
          ? ['return_approved', 'return_pickup_assigned', 'return_in_transit']
          : ['preparing', 'ready_for_pickup'];
        return (
          ['unassigned', 'assigned'].includes(dispatchStatus) &&
          actionableStatuses.includes(orderStatus)
        );
      });
  ...
  useEffect(() => {
    ...
    socketRef.current.on('new_ride_available', (rideData) => {
      ...
      const normalized = {
        ...
        orderId: rideData?.rideNumber || rideData?.rideId || rideData?.orderId,
        orderMongoId: rideData?.rideId || rideData?.orderMongoId,
        total: rideData?.fareEstimateTotal || rideData?.fare?.total,
        pickup: rideData?.pickup,
        drop: rideData?.drop,
        deliveryAddress: rideData?.drop,
      };
      if (ingestLiveEvent('delivery', normalized) === false) return;
    ...
    socketRef.current.on('order_cancelled', (statusData) => {
      ...
      const eventKey = getOrderAlertKey(statusData || {});
      const activeKey = getOrderAlertKey(activeOrderRef.current || {});
      if (eventKey && eventKey === activeKey) {
        debugLog('?? Removing cancelled order from local state');
        stopAlertLoop();
        activeOrderRef.current = null;
        setNewOrder(null);
        void stopRing();
    ...
    socketRef.current.on('ride_cancelled', (statusData) => {
      debugLog('?? Taxi ride cancelled event received via socket:', statusData);
      const eventKey = getOrderAlertKey({
        ...(statusData || {}),
        rideId: statusData?.rideId,
        orderMongoId: statusData?.rideId || statusData?.orderMongoId,
        orderId: statusData?.rideNumber || statusData?.orderId,
      });
      const activeKey = getOrderAlertKey(activeOrderRef.current || {});
      if (eventKey && eventKey === activeKey) {
        debugLog('?? Removing cancelled ride from local state');
        stopAlertLoop();
        activeOrderRef.current = null;
        setNewOrder(null);
        void stopRing();
    ...
    socketRef.current.on('ride_claimed', (data) => {
      debugLog('Taxi ride claimed by another partner:', data);
      const eventKey = getOrderAlertKey({
        ...(data || {}),
        rideId: data?.rideId,
        orderMongoId: data?.rideId || data?.orderMongoId,
        orderId: data?.rideNumber || data?.orderId,
      });
      const activeKey = getOrderAlertKey(activeOrderRef.current || {});
      if (eventKey && eventKey === activeKey) {
        stopAlertLoop();

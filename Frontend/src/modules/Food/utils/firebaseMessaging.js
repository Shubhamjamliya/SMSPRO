...
function normalizeModuleFromPath(pathname = window.location.pathname) {
  ...
  if (pathname.includes("/bike-rent/vendor")) return "bike_vendor";
  // Contractors run inside the same app but authenticate separately (BRD Q19),
  // so their pushes have to resolve against their own token, not the customer's.
  // Without this line a contractor falls through to "user" and registration is
  // silently skipped for anyone not also signed in as a customer.
  if (pathname.startsWith("/contractor")) return "contractor";
  return "user";
}

function getModuleAccessToken(moduleName) {
  const candidates = [`${moduleName}_accessToken`];
  if (moduleName === "seller") candidates.push("auth_seller");
  if (moduleName === "bike_vendor") candidates.push("auth_bike_vendor");
  if (moduleName === "contractor") candidates.push("auth_contractor");
...
async function saveOwnerFcmToken(contextModule, token, platform = "web") {
...
async function saveTokenByModule(moduleName, token, platform = "web") {
  ...
  if (moduleName === "seller" || moduleName === "bike_vendor") {
    await saveOwnerFcmToken(moduleName, token, platform);
    return;
  }
  if (moduleName === "user") {
...
export async function registerWebPushForCurrentModule(pathname = window.location.pathname) {
  ...
  const accessToken = getModuleAccessToken(moduleName);

...
import { getCartSubtotal, getLineAddonUnitTotal } from "@food/utils/cartTotals"
  ...
  return rawCart
    ...
    .map((item, index) => {
      ...
      return {
        ...
        // Per-item order limits (0 max = no cap). Kept on the line so cart
        // steppers know the bounds without re-fetching the menu.
        minOrderQuantity: Number(item.minOrderQuantity) > 0 ? Number(item.minOrderQuantity) : 1,
        maxOrderQuantity: Number(item.maxOrderQuantity) > 0 ? Number(item.maxOrderQuantity) : 0,
        // Server-priced add-ons for this line (name/price resolved backend-side).
        // Guest carts have no server figure yet, so derive it from the selection.
        addons: Array.isArray(item.addons) ? item.addons : [],
        addonUnitTotal:
          item.addonUnitTotal != null
            ? Number(item.addonUnitTotal) || 0
            : getLineAddonUnitTotal(item),
...
const extractCartPayload = (response) => {
  ...
  return {
    ...
    adjustedQuantities: Array.isArray(data.adjustedQuantities) ? data.adjustedQuantities : [],
    removedAddons: Array.isArray(data.removedAddons) ? data.removedAddons : [],
  }
}
...
export function CartProvider({ children }) {
  ...
  const applyServerCart = useCallback((payload) => {
    ...
    // The server re-clamps lines when a restaurant changes an item's limits
    // after it was added. Tell the customer rather than moving totals silently.
    const adjusted = Array.isArray(payload?.adjustedQuantities) ? payload.adjustedQuantities : []
    const droppedAddons = Array.isArray(payload?.removedAddons) ? payload.removedAddons : []
    droppedAddons.forEach((entry) => {
      toast.info(
        `${entry?.count > 1 ? `${entry.count} add-ons` : "An add-on"} on ${entry?.name || "an item"} is no longer available and was removed.`,
      )
    })
    adjusted.forEach((entry) => {
      const name = entry?.name || "An item"
      toast.info(
        entry?.reason === "max_order_quantity"
          ? `${name} is now limited to ${entry.to} per order — quantity updated.`
          : `${name} now requires a minimum of ${entry.to} — quantity updated.`,
      )
    })
    setCart(next)
    ...
    async (item, sourcePosition = null) => {
      ...
      if (!authed) {
        ...
        setCart((prev) => {
          ...
          // Guest carts are local-only, so mirror the same limits here — a signed
          // out customer sees the identical minimum/maximum behaviour.
          if (existing) {
            const topUp = Math.max(1, Number(item.quantity) || 1)
            return safePrev.map((i) =>
              i.id === lineId
                ? { ...i, quantity: clampQuantity(i.quantity + topUp, { ...item, ...i }) }
                : i,
            )
          }
          return [
            ...safePrev,
            {
              ...
              id: lineId,
              lineItemId: lineId,
              quantity: clampQuantity(Number(item.quantity) || getInitialQuantity(item), item),
            },
          ]
        })
        return { ok: true }
      ...
      try {
        const response = await foodCartAPI.addItem({
          ...
          // A bare "Add" tap sends the item's minimum (e.g. 4 for rasgulla);
          // the server clamps to the item's real limits regardless.
          quantity: Number(item.quantity) || getInitialQuantity(item),
          // Ids + quantities only — the server prices add-ons from the approved
          // catalogue and rejects anything that isn't currently available.
          addons: Array.isArray(item.addons)
            ? item.addons.map((addon) => ({
                addonId: String(addon.addonId || addon.id || addon._id || ""),
  ...
  const updateQuantity = useCallback(
    async (itemId, quantity, sourcePosition = null, productInfo = null) => {
      ...
      // Per-item order limits. Below the minimum there is no valid smaller
      // quantity, so a "−" tap at the minimum removes the line instead; above
      // the maximum the change is refused rather than silently trimmed.
      if (existingItem && quantity > 0) {
        const limits = getQuantityLimits(existingItem)
        if (quantity < limits.min) {
          // Only reachable when the item sets a minimum above 1.
          quantity = 0
        } else if (quantity > limits.max) {
          return {
            ok: false,
            code: "MAX_ORDER_QUANTITY",
            error: `You can order at most ${limits.max} of ${existingItem.name || "this item"}`,
          }
        }
      }
      ...
      if (!lineId) return { ok: false, error: "Cart item not found" }
  ...
  const cartForAnimation = useMemo(() => {
    ...
    const total = getCartSubtotal(normalizedCart)

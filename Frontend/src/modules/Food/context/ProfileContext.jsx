...
export function ProfileProvider({ children }) {
  ...
  const dedupeAddressesByLabel = (addressList = []) => {
    ...
    addressList.forEach((addr, index) => {
      ...
      const label = normalizedAddress.label
      // Home/Office stay unique by label; Other keeps each entry by id.
      const key =
        label === "Home" || label === "Office"
          ? label
          : (getAddressId(normalizedAddress) || `other-${index}`)
  ...
  const addAddress = useCallback(async (address) => {
    try {
      ...
      throw new Error(response?.data?.message || "Address was not saved")
    } catch (error) {
  ...
  const updateAddress = useCallback(async (id, updatedAddress) => {
    try {
      ...
      // Some APIs return success without body — refresh and resolve from list.
      const list = await refreshAddresses()
      const found = (list || []).find(
        (addr) => String(getAddressId(addr)) === String(id),
      )
      if (found) return found
      throw new Error(response?.data?.message || "Address was not updated")
    } catch (error) {

...
/** Just Order restaurant brand orange */
const THEME = "#FF6A00";
const THEME_RGB = "255, 106, 0";
...
const getOrderTotal = (orderLike) => {
  ...
  const items = getRestaurantVisibleItems(orderLike.items);
...
/**
 * Per-order accept card for the Live Orders → New Orders tab.
 */
export default function NewOrderAcceptCard({
  ...
  useEffect(() => {
    const seedPrep = Number(order?.preparationTime);
    if (Number.isFinite(seedPrep) && seedPrep >= 1) {
      setPrepTime(Math.min(180, Math.round(seedPrep)));
    } else {
      setPrepTime(11);
    }
  }, [order?.orderMongoId, order?._id, order?.orderId, order?.preparationTime]);
  ...
  return (
    <motion.div
      ...
      style={{
        ...
        borderWidth: "2px",
        borderTopWidth: "5px",
        boxShadow: `0 0 0 4px rgba(${THEME_RGB}, 0.12), 0 10px 28px -8px rgba(${THEME_RGB}, 0.35)`,
        background: `linear-gradient(180deg, rgba(${THEME_RGB}, 0.06) 0%, #ffffff 42%)`,
      }}
    >
      <div
        className="px-5 py-4 border-b flex items-center justify-between gap-3"
        style={{
          borderColor: `rgba(${THEME_RGB}, 0.18)`,
      ...
      >
        ...
        <div className="flex items-center gap-2 shrink-0">
          <div
            ...
            style={{
              borderColor: `rgba(${THEME_RGB}, 0.25)`,
              backgroundColor: `rgba(${THEME_RGB}, 0.08)`,
            }}
          >
        ...
        <div className="flex flex-col gap-3 md:col-span-5 md:pl-1">
          ...
          <div className="flex items-center justify-between bg-emerald-50/50 p-4 rounded-[20px] border border-emerald-100/50">
            ...
            <div className="flex items-center gap-4">
              ...
              <button
                type="button"
                onClick={() => setPrepTime(Math.min(180, prepTime + 1))}
                disabled={isAcceptingOrder || isExpired}
                className="w-8 h-8 flex items-center justify-center bg-white shadow-sm border border-emerald-100 rounded-full transition-all active:scale-90 text-emerald-600 disabled:opacity-50"
              >
          ...
          <div className="hidden md:flex flex-col gap-2 mt-auto pt-1">
            {isExpired ? (
              ...
              <button
                ...
                className="w-full h-12 rounded-2xl bg-[#FF6A00] hover:bg-[#E85D04] text-white font-black text-[14px] uppercase tracking-widest shadow-[0_8px_20px_rgba(255,106,0,0.3)] active:scale-[0.98] transition-all disabled:opacity-50 flex items-center justify-center"
              >
                {isAcceptingOrder
                  ? "Accepting..."
      ...
      <div className="px-5 pb-5 flex flex-col gap-3 md:hidden">
        {isExpired ? (
          ...
          >
            ...
            <motion.button
              type="button"
              className="absolute left-1.5 top-1/2 z-20 flex h-11 w-11 -translate-y-1/2 items-center justify-center rounded-[12px] bg-[#FF6A00] text-white shadow-lg active:scale-95 transition-transform"
              style={{
                x: (() => {
                  const { maxTravel } = getSliderMetrics();

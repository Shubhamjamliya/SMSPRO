...
const EMPTY = { name: "", icon: "", description: "", status: "active", displayOrder: 0 };
...
export default function Categories() {
  ...
  const load = async () => {
    setLoading(true);
    try {
      setRows(await constructionAdminApi.getCategories());
    } catch (error) {
  ...
  const save = async () => {
    ...
    try {
      ...
      if (editing) {
        await constructionAdminApi.updateCategory(editing._id, payload);
        toast.success("Category updated");
      } else {
        await constructionAdminApi.createCategory(payload);
  ...
  const remove = async () => {
    ...
    try {
      await constructionAdminApi.deleteCategory(deleting._id);
      toast.success("Category deleted");
      setDeleting(null);
      load();
    } catch (error) {
      // The API refuses to delete a category that still has services, and says
  ...
  const columns = [
    ...
    },
    {
      ...
      align: "center",
      cell: (row) => {
        const total = row.serviceCount ?? 0;
        const active = row.activeServiceCount ?? 0;
        // An active category with no live services is invisible to customers —
        // say so here rather than letting an admin publish a dead end.
        const isDeadEnd = row.status === "active" && active === 0;
        return (
          <div className="text-sm">
            <span className="font-semibold text-gray-900 tabular-nums">{active}</span>
            <span className="text-gray-400"> / {total}</span>
            {isDeadEnd ? (
              <p className="mt-0.5 text-[11px] font-medium text-amber-600">
                Hidden — no live services
              </p>
            ) : null}
          </div>
        );
          ...
          <div className="flex items-start justify-between gap-2">
            <div className="min-w-0">
              ...
              <p className="mt-0.5 text-xs text-gray-500">
                {row.activeServiceCount ?? 0} live of {row.serviceCount ?? 0} services
              </p>
            </div>
  ...
  return (
    <div className={CN_ADMIN_PAGE_CLASS}>
      <PageHeader
        ...
        title="Categories"
        description="The top level of the service list customers browse. Add and edit these yourself — no software release needed."
        actions={
      ...
      <AdminTable
        ...
        emptyState={{
          ...
          description: search
            ...
            : "Start with a few broad categories — New Building, Renovation, Interior Fit-out — then add services under each.",
        }}
      />

      <Dialog open={modalOpen} onOpenChange={setModalOpen}>
        <DialogContent className={CN_DIALOG_CONTENT_CLASS}>
          ...
          <div className="min-h-0 flex-1 space-y-4 overflow-y-auto overscroll-contain px-4 py-4 sm:px-5">
            <Input
              ...
              placeholder="e.g. Renovation & Remodelling"
              value={form.name}
              error={errors.name}
            ...
            <MediaUploadField
              ...
              helperText="Optional — shown on the category card in the customer app"
              value={form.icon}
              onChange={(next) => setField("icon", next)}
              folder="construction/categories"
            ...
            <label className="block text-sm">
              ...
              <Textarea
                placeholder="What kind of work this covers, in a sentence"
                rows={3}
        ...
        <DialogContent className={`${CN_DIALOG_CONTENT_CLASS} max-w-md`}>
          ...
          <div className="px-4 py-4 sm:px-5">
            <p className="text-sm text-gray-600">
              Delete <strong className="text-gray-900">{deleting?.name}</strong>? This cannot be
              undone. Categories that still have services cannot be deleted — move or delete those
              first.
            </p>
          </div>
          <DialogFooter className="shrink-0 gap-2 border-t border-gray-100 px-4 py-3 sm:px-5">

...
import { filterBySearch, paginateItems } from "../utils/porterTableHelpers";
...
const Zones = () => {
  ...
  const loadZones = useCallback(async () => {
    setLoading(true);
    try {
      const data = await porterAdminApi.getZones({
        limit: 100,
  ...
  const handleDelete = async (row) => {
    ...
    try {
      await porterAdminApi.deleteZone(row.id);
    ...
    },
    {
      ...
      cell: (row) => (
        <div className="flex justify-end gap-1">
          <Button variant="ghost" size="sm" onClick={() => navigate(`/admin/porter/zones/view/${row.id}`)} title="View">
            <Eye size={14} />
          </Button>
          <Button variant="ghost" size="sm" onClick={() => navigate(`/admin/porter/zones/edit/${row.id}`)} title="Edit">
            <Pencil size={14} />
          </Button>
  ...
  return (
    <div className="just-order-theme-scope space-y-6 max-w-[90rem] mx-auto px-4 sm:px-6 lg:px-8 py-8 pb-24">
      <PageHeader
        title="Porter Service Zones"
        description="Draw service areas on the map — same flow as Food zone setup"
        actions={
          <Button onClick={() => navigate("/admin/porter/zones/add")}>

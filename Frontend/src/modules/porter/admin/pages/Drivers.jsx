...
const Drivers = () => {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const data = await porterAdminApi.getDrivers({
        limit: 100,
  ...
  const toggleStatus = async (row) => {
    ...
    setSaving(true);
    try {
      await porterAdminApi.updateDriverStatus(row.id, next);
      ...
      cell: (row) => (
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-gray-500 font-mono">{String(row.id || "").slice(-8)}</p>
          </div>
        </div>
      ),
    },
  ...
  return (
    <div className="just-order-theme-scope space-y-6 max-w-[90rem] mx-auto px-4 sm:px-6 lg:px-8 py-8 pb-24">
      <PageHeader title="Drivers" description="Partners authorized for the Porter module" />
      ...
      <SectionCard flush>
        <div className="p-4 space-y-4">
          ...
          <AdminTable
            ...
            emptyState={{ title: "No Porter drivers yet", description: "Drivers appear here once they are authorized for the Porter module." }}
          />
        </div>
      </SectionCard>

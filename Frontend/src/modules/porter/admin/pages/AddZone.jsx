...
export default function AddZone() {
  ...
  useEffect(() => {
    return () => {
      ...
      if (mapRef.current?.__porterZoneResizeObserver) {
        mapRef.current.__porterZoneResizeObserver.disconnect();
        delete mapRef.current.__porterZoneResizeObserver;
      }
    };
  }, []);
  ...
  const fetchExistingZones = async () => {
    try {
      const data = await porterAdminApi.getZones({ limit: 1000 })
  ...
  const fetchZone = async () => {
    try {
      setLoading(true)
      const zoneData = await porterAdminApi.getZoneById(id)
      if (zoneData) {
        setFormData({
  ...
  const initializeMap = (google) => {
    ...
    if (resizeObserver && mapRef.current) {
      ...
      mapRef.current.__porterZoneResizeObserver = resizeObserver
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (isEditMode && id) {
        await porterAdminApi.updateZone(id, zoneData)
        toast.success("Zone updated successfully")
      } else {
        await porterAdminApi.createZone(zoneData)
  ...
  return (
    <FormPageShell
      title={isEditMode ? "Edit porter zone" : "Add porter zone"}
      ...
      onBack={() => navigate("/admin/porter/zones")}
    ...
    >
      <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div className="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
              <div className="flex flex-col gap-3 border-b border-slate-100 p-3 sm:flex-row sm:items-center sm:justify-between sm:p-4">
                <div>
                  ...
                  <p className="text-xs text-slate-500">Draw the polygon that defines this porter zone</p>
                </div>
        ...
        <FormActions
          ...
          onCancel={() => navigate("/admin/porter/zones")}
          submitLabel={

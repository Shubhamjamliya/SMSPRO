...
const SearchPage = () => {
    ...
    // ── Location / zone scope (same as Home catalog filtering) ────────────────
    const hasValidLocation = useMemo(
        () => hasQuickZoneScope(currentLocation),
        [currentLocation?.latitude, currentLocation?.longitude],
    );

    const zoneCatalogParams = useMemo(
    ...
    }, [hasValidLocation, debouncedQuery, zoneCatalogParams]);
    ...
    useEffect(() => {
        ...
        return () => { cancelled = true; };
    }, [debouncedQuery, hasValidLocation, zoneCatalogParams]);
    ...
    return (
        <div className="min-h-screen bg-[#F5F7F8] dark:bg-background font-outfit transition-colors duration-500">
            ...
            <div className="w-full px-4 py-4 space-y-6 pb-28">
                {debouncedQuery ? (
                    /* ── Search Results ── */
                    <section>
                        <div className="flex justify-between items-center mb-4">
                            <h2 className="text-lg font-black text-slate-800 dark:text-slate-200 tracking-tight transition-colors">
                                Search Results
                            </h2>

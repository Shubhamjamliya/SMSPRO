...
const mapProduct = (p) => ({
  ...
  sellerId: p.sellerId?._id || p.sellerId || p.seller?._id || p.seller?.id || null,
});

const ShopByStorePage = () => {
  const { currentLocation } = useAppLocation();
  const [sections, setSections] = useState([]);
  const [zoneSellerIds, setZoneSellerIds] = useState(null);
  ...
  useEffect(() => {
    const load = async () => {
      if (!hasQuickZoneScope(currentLocation)) {
        setIsLoading(false);
        setSections([]);
        setZoneSellerIds(null);
        setActiveStoreId(null);
        return;
      }

      setIsLoading(true);
      const zoneParams = buildQuickZoneCatalogParams(currentLocation);
      try {
        const [sectionsRes, storesRes] = await Promise.all([
          customerApi.getOfferSections(zoneParams).catch(() => ({ data: {} })),
          customerApi
            .getStores({ ...zoneParams, page: 1, limit: 100 })
            .catch(() => ({ data: {} })),
        ]);

        const list =
          sectionsRes.data?.results || sectionsRes.data?.result || sectionsRes.data || [];
        const normalized = Array.isArray(list) ? list : [];
        setSections(normalized);

        const shops =
          storesRes?.data?.result?.shops ||
          storesRes?.data?.result?.items ||
  ...
  const activeProducts = useMemo(() => {
    ...
    const mapped = raw.filter(Boolean);
    // Zone filter: keep only products whose seller is in the current zone.
    if (!zoneSellerIds || zoneSellerIds.size === 0) return mapped;
    return mapped.filter((p) => {
      const sid = p.sellerId ? String(p.sellerId) : "";
      if (!sid) return true; // keep untagged products rather than wiping the aisle
      return zoneSellerIds.has(sid);
    });
  }, [activeStore, zoneSellerIds]);

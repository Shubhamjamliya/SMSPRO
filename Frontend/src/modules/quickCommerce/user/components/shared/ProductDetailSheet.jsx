...
const PRODUCT_DETAILS_TEMPLATE = (supportEmail) => [
...
// ─── Animation variants defined outside component (stable references) ─────────
...
const desktopModalTransition = { type: 'spring', damping: 28, stiffness: 380 };
...
// ─── RTF cleaner (pure function, no deps) ────────────────────────────────────
...
const StarRow = React.memo(({ rating, size = 10 }) => (
    <div className="flex gap-0.5">
        {STAR_RATINGS.map((i) => (
            <Star
                ...
                size={size}
                className={cn(
                    i <= rating ? 'text-red-400 fill-red-400' : 'text-gray-200 dark:text-slate-800'
                )}
            />
        ))}
    </div>
...
const ReviewCard = React.memo(({ r, idx }) => (
    <motion.div
    ...
    >
        <div className="flex justify-between items-start">
            <div className="flex items-center gap-2">
                <div className="h-7 w-7 rounded-full bg-gradient-to-br from-green-100 to-emerald-50 dark:from-emerald-900 dark:to-green-950 flex items-center justify-center text-[9px] font-[800] text-[#0c831f] dark:text-emerald-400 ring-1 ring-green-50 dark:ring-white/5">
                    {r.userId?.name?.[0] || 'A'}
                </div>
                <div>
                    <p className="text-[11px] font-[700] text-gray-800 dark:text-slate-200">
                        {r.userId?.name || 'Anonymous'}
                    </p>
                    <StarRow rating={r.rating} size={9} />
                </div>
            </div>
            <span className="text-[9px] font-[600] text-gray-400">
                {new Date(r.createdAt).toLocaleDateString()}
            </span>
        </div>
        <p className="text-[11px] text-gray-500 dark:text-slate-400 font-[500] leading-relaxed pl-9">
            {r.comment}
        </p>
    </motion.div>
));

const MobileReviewCard = React.memo(({ r }) => (
    <div className="p-5 rounded-2xl border border-gray-100 space-y-2">
        <div className="flex justify-between items-start">
            <div className="flex items-center gap-2">
...
const ProductDetailSheet = () => {
    ...
    const shopOpenState = useMemo(
        () => resolveShopOpenState(selectedProduct),
        [selectedProduct],
    );
    const shopClosed = Boolean(selectedProduct) && !shopOpenState.isOpen;
    ...
    const allImages = useMemo(() => {
        ...
        if (selectedProduct.mainImage) imgs.push(selectedProduct.mainImage);
        else if (selectedProduct.image) imgs.push(selectedProduct.image);
        if (Array.isArray(selectedProduct.galleryImages)) {
            imgs.push(...selectedProduct.galleryImages);
        }
        const unique = [...new Set(imgs.filter(Boolean))];
        return unique.length > 0 ? unique : [FALLBACK_IMAGE];
    }, [selectedProduct, selectedVariant]);

    // Real seller/product fields (fallback care email)
    const productDetails = useMemo(() => {
        if (!selectedProduct) return PRODUCT_DETAILS_TEMPLATE(supportEmail);
        const rows = [];
        if (selectedProduct.brand) {
            rows.push({ label: 'Brand', value: selectedProduct.brand, emoji: '🏷️' });
        }
        const unit = selectedProduct.unit || selectedProduct.weight;
        if (unit) rows.push({ label: 'Unit', value: unit, emoji: '📦' });
        if (Number(selectedProduct.packingAmount) > 0) {
            rows.push({
                label: 'Packing',
                value: `₹${Number(selectedProduct.packingAmount)}`,
                emoji: '🛍️',
            });
        }
        if (Number(selectedProduct.returnWindowDays) > 0) {
            rows.push({
                label: 'Return',
                value: `${Number(selectedProduct.returnWindowDays)} days`,
                emoji: '↩️',
            });
        }
        if (selectedVariant?.sku) {
            rows.push({ label: 'SKU', value: selectedVariant.sku, emoji: '🔢' });
        }
        const stock = Number(
        ...
        rows.push({ label: 'Customer Care', value: supportEmail, emoji: '📧' });
        return rows.length > 0 ? rows : PRODUCT_DETAILS_TEMPLATE(supportEmail);
    }, [selectedProduct, selectedVariant, supportEmail]);
    ...
    }, [selectedProduct, productId, selectedVariant]);

    const cartItem = useMemo(() => {
        if (!selectedProduct || !cartLineId) return null;
        return cart.find((item) => {
            const itemId = String(
                item.cartLineId || item.id || item._id || item.productId || item.itemId || '',
            );
            return itemId === cartLineId;
        });
    }, [cart, selectedProduct, cartLineId]);

    const quantity = cartItem?.quantity ?? 0;
    const isWishlisted = useMemo(
        () => (selectedProduct ? isInWishlist(selectedProduct, selectedVariant) : false),
        [selectedProduct, selectedVariant, isInWishlist]
    );

    // Memoised rating average
    ...
    // Cleaned description (changes only when product changes)
    ...
    const productDiscount = useMemo(() => {
        ...
        const { originalPrice, price } = selectedProduct;
        return originalPrice > price && originalPrice > 0
            ? Math.round(((originalPrice - price) / originalPrice) * 100)
            : null;
    }, [selectedProduct]);

    const activeDiscount = variantDiscount ?? productDiscount;
    ...
    const fetchReviews = useCallback(async (pid) => {
        try {
            ...
            const res = await customerApi.getProductReviews(pid);
            if (res.data.success) setReviews(res.data.results);
        } catch (err) {
    ...
    useEffect(() => {
        ...
        const preferredKey = String(
          getVariantKey(selectedProduct.selectedVariant) ||
        ...
        const preferred = preferredKey
          ...
          : null;
        setSelectedVariant(preferred || (variants.length > 0 ? variants[0] : null));
        setActiveImageIndex(0);
        if (productId) fetchReviews(productId);
    }, [selectedProduct, productId, fetchReviews]);
    ...
    }, [toggleWishlistGlobal, selectedProduct, selectedVariant, isWishlisted, showToast]);

    const handleAddToCart = useCallback(async () => {
        const shopState = resolveShopOpenState(selectedProduct);
        if (!shopState.isOpen) {
        ...
        const cartProduct = hasVariants
            ? applyVariantToProduct(selectedProduct, selectedVariant)
            : selectedProduct;
    ...
    }, [addToCart, selectedProduct, selectedVariant, showToast]);

    const handleIncrement = useCallback(() => {
        const stock = Number(selectedVariant?.stock ?? selectedProduct?.stock ?? 0);
        if (quantity >= stock) {
            showToast(`Only ${stock} items are available in stock.`, "error");
            return;
        }
        updateQuantity(cartLineId || productId, 1);
    }, [updateQuantity, cartLineId, productId, selectedProduct, selectedVariant, quantity, showToast]);

    const handleDecrement = useCallback(() => {
        const lineId = cartLineId || productId;
        if (quantity === 1) removeFromCart(lineId);
        else updateQuantity(lineId, -1);
    }, [quantity, removeFromCart, updateQuantity, cartLineId, productId]);
    ...
    const handleReviewCommentChange = useCallback(
        (e) => setNewReview((prev) => ({ ...prev, comment: e.target.value })),
        []
    );
    ...
    }, [newReview, productId, showToast]);

    // Star rating setter — stable per star value
    const makeRatingSetter = useCallback(
        (s) => () => setNewReview((prev) => ({ ...prev, rating: s })),
        []
    );
    ...
    const strikePrice = selectedVariant
        ? (Number(selectedVariant.salePrice) > 0 ? selectedVariant.price : selectedProduct.originalPrice)
        : selectedProduct.originalPrice;
    const hasDiscount =
        Number(strikePrice) > Number(currentPrice) &&
        Number(currentPrice) > 0 &&
        (activeDiscount !== null ? activeDiscount > 0 : true);
                ...
                <>
                    ...
                    >
                        <div className="flex w-full min-h-full">
                                ...
                                <div className="absolute top-0 left-0 right-0 flex items-center justify-between p-5 z-20">
                                    ...
                                    {productDiscount !== null && (
                                        <motion.div
                                            ...
                                            className="bg-gradient-to-r from-[#FF6A00] to-[#ff8533] text-white text-[10px] font-[800] px-3 py-1.5 rounded-xl uppercase tracking-wider shadow-md shadow-orange-200/40"
                                        >
                                            {productDiscount}% OFF
                                        </motion.div>
                                    )}
                                    ...
                                    <motion.button
                                        ...
                                        className={cn(
                                            ...
                                            isWishlisted
                                                ? 'bg-red-50/95 dark:bg-red-950/30 border-red-100 dark:border-red-900'
                                                : 'bg-white/95 dark:bg-slate-800/95 border-gray-100/80 dark:border-white/10'
                                        )}
                                    >
                                    ...
                                    {allImages.length > 1 && (
                                        <div className="flex flex-col gap-2 px-3 py-2 overflow-y-auto no-scrollbar">
                                            {allImages.slice(0, 5).map((img, i) => (
                                                ...
                                                >
                                                    <img src={img} alt="" loading="lazy" className="w-full h-full object-contain p-1.5 mix-blend-multiply dark:mix-blend-normal" />
                                                </motion.button>
                                            ))}
                                        </div>
                                    )}
                                        ...
                                        {allImages.map((_, i) => (
                                            <button
                                                ...
                                                className={cn(
                                                    ...
                                                    i === activeImageIndex
                                                        ? 'w-8 h-2 bg-[#0c831f]'
                                                        : 'w-2 h-2 bg-gray-300/60 dark:bg-slate-700/60 hover:bg-gray-400'
                                                )}
                                            />
                                        ))}
                                    </div>
                            ...
                            {/* Right: Product Info */}
                            <div className="flex-1 flex flex-col bg-white dark:bg-card transition-colors duration-500 overflow-y-auto">
                                <div className="flex-1 px-7 py-6 lg:px-8 lg:py-7 space-y-3">
                                    ...
                                    <div className="flex items-center gap-2 flex-wrap">
                                        ...
                                        <motion.div
                                            ...
                                            className="flex items-center gap-1 px-2.5 py-1.5 bg-red-50 dark:bg-red-950/30 text-red-600 dark:text-red-400 rounded-lg text-[10px] font-[700] border border-red-100/50 dark:border-red-900/30"
                                        >
                                            <Star size={10} fill="currentColor" />
                                            {avgRating}
                                            <span className="text-red-400 dark:text-red-500 font-medium">({reviewCount})</span>
                                        </motion.div>
                                    </div>
                                        ...
                                        <div className="flex items-center gap-3 mb-2">
                                            ...
                                            <div className="flex items-center gap-1.5 px-2.5 py-1 bg-slate-50 dark:bg-slate-800 border border-slate-100 dark:border-white/5 rounded-full">
                                                ...
                                                <span className="text-[11px] font-bold text-slate-500 dark:text-slate-400">
                                                    Sold by: <span className="text-foreground">{storeName}</span>
                                                </span>
                                                <div className="flex items-center justify-center w-3 h-3 bg-[#0c831f] rounded-full text-white">
                                                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="4" strokeLinecap="round" strokeLinejoin="round" className="w-2 h-2"><polyline points="20 6 9 17 4 12" /></svg>
                                                </div>
                                            </div>
                                        </div>
                                    </motion.div>
                                        ...
                                        <div className="relative flex items-center justify-between py-4 px-5">
                                            <div className="flex flex-col gap-1">
                                                <div className="flex items-baseline gap-2">
                                                    <span className="text-[28px] lg:text-[32px] font-[800] text-[#FF6A00] tracking-tight leading-none">
                                                        ₹{currentPrice}
                                                    </span>
                                                    {hasDiscount && (
                                                        <span className="text-[14px] text-gray-400 line-through font-[600]">₹{strikePrice}</span>
                                                    )}
                                                </div>
                                                {hasDiscount && (
                                            ...
                                            <div>
                                                {shopClosed ? (
                                                    <div className="rounded-xl border border-slate-300 bg-slate-100 px-3 py-2 text-[11px] font-bold text-slate-700 max-w-[220px]">
                                                        <span className="mr-1 inline-flex rounded-full bg-slate-800 px-1.5 py-0.5 text-[9px] uppercase text-white">Off</span>
                                                        Shop closed
                                                        {shopOpenState.openingHours
                                                            ? ` · ${formatOpeningHoursAMPM(shopOpenState.openingHours)}`
                                                            : ""}
                                                    </div>
                                                ) : quantity > 0 ? (
                                                    <div className="flex items-center gap-1 bg-white dark:bg-slate-800 border border-orange-200 dark:border-orange-800/30 rounded-xl p-1 shadow-sm transition-colors">
                                                        <motion.button whileTap={{ scale: 0.85 }} onClick={handleDecrement} className="w-9 h-9 bg-orange-50 dark:bg-orange-900/30 rounded-lg flex items-center justify-center text-[#FF6A00] hover:bg-orange-100 dark:hover:bg-orange-900/50 transition-colors">
                                                            <Minus size={16} strokeWidth={2.5} />
                                                        </motion.button>
                                                        <span className="font-[800] text-base text-gray-800 dark:text-slate-200 w-8 text-center">{quantity}</span>
                                                        <motion.button whileTap={{ scale: 0.85 }} onClick={handleIncrement} className="w-9 h-9 bg-[#FF6A00] rounded-lg flex items-center justify-center text-white hover:bg-[#e85f00] transition-colors shadow-sm">
                                                    ...
                                                    <motion.button
                                                        ...
                                                        className="bg-gradient-to-r from-[#FF6A00] to-[#e85f00] text-white h-11 px-6 rounded-xl font-[800] text-[13px] flex items-center gap-2 shadow-md shadow-orange-200/50 hover:shadow-lg transition-all uppercase tracking-wide border border-orange-700/20"
                                                    >
                                    ...
                                    {cartCount > 0 && (
                                        ...
                                        >
                                            <Link
                                                to={cartPath}
                                                onClick={closeProduct}
                                                className="w-[80%] bg-gradient-to-r from-[#FF6A00] to-[#e85f00] text-white h-[40px] rounded-xl flex items-center justify-between px-4 shadow-md shadow-orange-200/40 hover:shadow-lg hover:-translate-y-0.5 transition-all active:scale-[0.98]"
                                            >
                                                <div className="flex items-center gap-2">
                                        ...
                                        >
                                            ...
                                            <div className="flex gap-3 flex-wrap">
                                                {selectedProduct.variants.map((v, idx) => {
                                                    const selected = getVariantKey(selectedVariant) === getVariantKey(v);
                                                    const label = getVariantDisplayLabel(v, selectedProduct) || v.name || v.sku || `Option ${idx + 1}`;
                                                    return (
                                                    <motion.button
                                                        key={getVariantKey(v) || idx}
                                                        ...
                                                        className={cn(
                                                            ...
                                                            selected
                                                                ? 'bg-orange-50 dark:bg-orange-950/30 border-[#FF6A00] text-[#FF6A00] shadow-md shadow-orange-100/50 dark:shadow-none'
                                    ...
                                    {cleanDesc && (
                                        ...
                                        >
                                            <h3 className="font-[800] text-gray-900 dark:text-slate-100 mb-3 text-[14px] tracking-tight flex items-center gap-2">
                                                <span className="w-1 h-3.5 bg-[#FF6A00] rounded-full" />
                                                About this product
                                            </h3>
                                            <div
                                    ...
                                    >
                                        <h3 className="font-[700] text-gray-900 dark:text-slate-100 mb-2.5 text-[14px] tracking-tight flex items-center gap-2">
                                            <span className="w-0.5 h-4 bg-[#FF6A00] rounded-full" />
                                            Product Details
                                        </h3>
                                    ...
                                    >
                                        <h3 className="text-[14px] font-[700] text-gray-900 dark:text-slate-100 flex items-center justify-between tracking-tight">
                                            <span className="flex items-center gap-2">
                                                <span className="w-0.5 h-4 bg-red-400 rounded-full" />
                                                Customer Reviews
                                            </span>
                                            <div className="flex items-center gap-1 px-2.5 py-1 bg-red-50 dark:bg-red-950/30 text-red-600 dark:text-red-400 rounded-lg text-[10px] font-[700] border border-red-100/50 dark:border-red-900/30">
                                                <Star size={10} fill="currentColor" />
                                                {avgRating}
                                                <span className="text-red-400 dark:text-red-500 font-[500] text-[9px]">({reviewCount} reviews)</span>
                                            </div>
                                        </h3>
                                        ...
                                        <div className="bg-gradient-to-br from-slate-50/80 to-gray-50/80 dark:from-slate-900/80 dark:to-slate-950/80 p-4 rounded-xl border border-slate-100/70 dark:border-white/5 transition-colors">
                                            ...
                                            <form onSubmit={handleReviewSubmit} className="space-y-2.5">
                                                <div className="flex gap-1.5">
                                                    {STAR_RATINGS.map((s) => (
                                                        <motion.button
                                                            ...
                                                            whileTap={{ scale: 0.9 }}
                                                            onClick={makeRatingSetter(s)}
                                                            className={cn(
                                                                'h-8 w-8 rounded-lg flex items-center justify-center transition-all',
                                                                newReview.rating >= s
                                                                    ? 'bg-red-100 dark:bg-red-950/50 text-red-500 shadow-sm shadow-red-100 dark:shadow-none'
                                                                    : 'bg-white dark:bg-slate-800 text-gray-300 dark:text-slate-600 border border-gray-100 dark:border-white/5 hover:border-red-200'
                                                            )}
                                                        >
                                                ...
                                                <textarea
                                                    ...
                                                    className="w-full bg-white dark:bg-slate-800 border border-gray-100 dark:border-white/5 rounded-lg p-3 text-[12px] font-[500] min-h-[70px] outline-none focus:border-[#0c831f]/40 focus:ring-2 focus:ring-green-50 dark:focus:ring-green-900/20 transition-all resize-none dark:text-slate-200"
                                                />
                                                <Button
                                                    type="submit"
                                                    disabled={isSubmittingReview}
                                                    className="w-full h-9 bg-gray-900 dark:bg-foreground hover:bg-gray-800 dark:text-background font-[700] rounded-lg text-[10px] uppercase tracking-widest transition-all hover:shadow-md active:scale-[0.98]"
                                                >
                                        ...
                                        <div className="space-y-2.5">
                                            {reviewLoading ? (
                                                ...
                                                reviews.map((r, rIdx) => (
                                                    <ReviewCard key={r._id} r={r} idx={rIdx} />
                                                ))
                                            ) : (
                    ...
                    <motion.div
                        ...
                        initial={{
                            opacity: 0, scale: 0.9, y: '100vh',
                            top: '10%', bottom: '10%',
                            left: '50%', x: '-50%',
                            width: 'min(90%, 400px)', borderRadius: '24px',
                        }}
                        animate={{
                        ...
                        className="md:hidden fixed z-[230] bg-white dark:bg-card shadow-2xl overflow-hidden flex flex-col"
                            ...
                            <div className="px-5 pt-2 pb-6">
                                ...
                                <div className="flex items-center gap-1.5 mb-4">
                                    ...
                                    <span className="text-[11px] font-bold text-gray-500 uppercase tracking-widest">
                                        Sold by: <span className="text-[#1A1A1A]">{storeName}</span>
                                    </span>
                                </div>
                                            ...
                                            {selectedProduct.variants.map((v, idx) => {
                                                ...
                                                <button
                                                    ...
                                                    className={cn(
                                                        ...
                                                        selected
                                                            ? 'bg-white border-2 border-[#FF6A00] text-[#FF6A00] shadow-sm shadow-orange-100'
                                                            : 'bg-gray-50 border border-gray-200 text-gray-600'
                                                    )}
                                                >
                                                    {label}
                                                    {selected && (
                                                        <div className="absolute top-0 right-0 w-3 h-3 bg-[#FF6A00] rounded-bl-lg" />
                                                    )}
                                                </button>
                                                );
                                            })}
                                ...
                                <div className="space-y-4">
                                    ...
                                    <div className="grid grid-cols-2 gap-4 text-xs">
                                        {productDetails.map((d) => (
                                            <div key={d.label} className="bg-gray-50 p-3 rounded-lg">
                                                <span className="text-gray-400 block mb-1">{d.label}</span>
                                ...
                                <div className="space-y-6">
                                    <h3 className="text-xl font-black text-gray-900 flex items-center justify-between">
                                        Customer Reviews
                                        <div className="flex items-center gap-1.5 px-2.5 py-1 bg-red-50 text-red-600 rounded-lg text-xs font-bold">
                                    ...
                                    <div className="space-y-4">
                                        {reviewLoading ? (
                                            ...
                                            reviews.map((r) => <MobileReviewCard key={r._id} r={r} />)
                                        ) : (
                                ...
                                <div className="flex items-center justify-between gap-4">
                                    <div className="flex flex-col">
                                        <div className="flex items-center gap-2">
                                            ...
                                            <span className="bg-orange-100 text-[#FF6A00] text-[10px] font-black px-1.5 py-0.5 rounded">
                                                {activeDiscount || 20}% OFF
                                            </span>
                                        </div>
                                        <div className="text-2xl font-black text-[#1A1A1A]">₹{currentPrice}</div>
                                    </div>

                                    {shopClosed ? (
                                        <div className="flex-1 rounded-xl border border-slate-300 bg-slate-100 h-[52px] px-3 flex items-center justify-center text-xs font-bold text-slate-700 text-center">
                                            Shop closed — ordering paused
                                            {shopOpenState.openingHours
                                                ? ` (${formatOpeningHoursAMPM(shopOpenState.openingHours)})`
                                                : ""}
                                        </div>
                                    ) : quantity > 0 ? (
                                        <div className="flex items-center gap-3 bg-white border-2 border-[#FF6A00] rounded-xl p-1.5 shadow-lg shadow-orange-100 flex-1 justify-between max-w-[180px]">
                                            <motion.button whileTap={{ scale: 0.9 }} onClick={handleDecrement} className="w-10 h-10 bg-orange-50 rounded-lg flex items-center justify-center text-[#FF6A00] hover:bg-orange-100 transition-colors">
                                                <Minus size={18} strokeWidth={3} />
                                            </motion.button>
                                            <span className="font-black text-lg text-gray-800 w-8 text-center">{quantity}</span>
                                            <motion.button whileTap={{ scale: 0.9 }} onClick={handleIncrement} className="w-10 h-10 bg-[#FF6A00] rounded-lg flex items-center justify-center text-white hover:bg-[#e85f00] transition-colors shadow-md shadow-orange-100">
                                        ...
                                        <motion.button
                                            whileTap={{ scale: 0.95 }}
                                            onClick={handleAddToCart}
                                            className="flex-1 bg-[#FF6A00] text-white h-[52px] rounded-xl font-black text-base flex items-center justify-center gap-2 shadow-xl shadow-orange-100 hover:bg-[#e85f00] transition-all"
                                        >
                                            ADD TO CART
                                        </motion.button>
                                    )}
                                ...
                                {cartCount > 0 && (
                                    ...
                                    >
                                        <Link
                                            to={cartPath}
                                            onClick={closeProduct}
                                            className="w-full bg-[#FF6A00] text-white h-[56px] rounded-xl flex items-center justify-between px-5 shadow-lg shadow-orange-100/50 hover:bg-[#e85f00] transition-all active:scale-[0.98]"
                                        >

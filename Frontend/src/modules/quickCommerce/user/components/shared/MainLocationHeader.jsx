...
const CategoryNavColumn = React.memo(function CategoryNavColumn({
  ...
  const IconComponent = cat.icon || getCategoryIconComponent(cat.iconId);
  const svgMarkup = !IconComponent || IconComponent === Sparkles
  ...
  return (
    ...
    >
      <div className="relative z-10 flex h-9 w-9 items-center justify-center md:h-11 md:w-11">
        {svgMarkup ? (
          <div
            className="h-5 w-5 text-white md:h-6 md:w-6 [&>svg]:h-full [&>svg]:w-full"
      ...
      setInternalCategories(
        dbCats
          ...
          .map((cat) => ({
            ...cat,
            id: cat._id,
            icon: getCategoryIconComponent(cat.iconId) || Sparkles,
          })),
      );
    });

...
const OrdersList = () => {
        ...
        try {
            ...
            if (response.data.success) {
                ...
                const formatted = dbOrders.map(o => ({
                    ...
                    amount: (() => {
                        return Number(o.amount) || Number(o.total) || Number(o.pricing?.total) || 0;
                    })(),
                    ...
                    hasReturn: orderHasReturn(o),
                    resolvedReturnStatus: resolveReturnStatus(o),
    ...
    const stats = useMemo(() => {
        const totalEarnings = safeOrders.reduce((sum, o) => {
            if (['cancelled', 'returned'].includes(String(o.status).toLowerCase())) {
                return sum;
            }
            return sum + o.amount;
        }, 0);
        ...
        if (status === 'returned') {
            const returnedOrders = safeOrders.filter((o) => o.hasReturn);
            const refundToCustomers = returnedOrders.reduce(
                (sum, o) => sum + Number(o.returnInfo?.refundAmount || 0),
                0,
            );
            const refundsPaid = returnedOrders.reduce(
                (sum, o) => sum + Number(o.returnInfo?.refundedAmount || 0),
                0,
            );
            const recoveredFromSellers = returnedOrders.reduce(
                (sum, o) => sum + Number(o.returnInfo?.sellerRecovered || 0),
                0,
            );
            const platformPickupCost = returnedOrders.reduce(
                (sum, o) => sum + Number(o.returnInfo?.pickupFeeAdminExpense || 0),
                0,
            );

            return [
                { label: 'Returned Orders', value: returnedOrders.length, icon: RotateCcw, color: 'rose' },
                {
                    label: 'Refund To Customers',
                    value: `₹${refundToCustomers.toLocaleString('en-IN')}`,
                    subLabel: `₹${refundsPaid.toLocaleString('en-IN')} paid out`,
                    icon: IndianRupee,
                    color: 'amber',
                },
                {
                    label: 'Recovered From Sellers',
                    value: `₹${recoveredFromSellers.toLocaleString('en-IN')}`,
                    icon: CheckCircle2,
                    color: 'emerald',
                },
                {
                    label: 'Platform Pickup Cost',
                    value: `₹${platformPickupCost.toLocaleString('en-IN')}`,
                    icon: Truck,
                    color: 'blue',
                },
            ];
    ...
    const getStatusStyles = (status) => {
        switch (status.toLowerCase()) {
            ...
            case 'packed': return 'bg-indigo-100 text-indigo-600 border-primary/20';
    ...
    const handleExport = () => {
        ...
        const rows = [
            ...
            ...filteredOrders.map((order) => [
                ...
                order.payment,
                order.date,
                order.hasReturn ? getReturnStatusLabel(order.resolvedReturnStatus) : '',
                order.hasReturn ? getRefundStatusLabel(order.returnInfo?.refundStatus) : '',
                order.hasReturn ? Number(order.returnInfo?.refundAmount || 0) : '',
                order.hasReturn ? Number(order.returnInfo?.sellerRecovered || 0) : '',
            ]),
        ];
                ...
                {stats.map((stat, i) => (
                    <Card key={i} className="p-5 border-none shadow-sm ring-1 ring-slate-100 bg-white group hover:ring-fuchsia-200 transition-all text-left">
                        ...
                        <div>
                            ...
                            {stat.subLabel && (
                                <p className="text-[10px] font-bold text-slate-400 mt-1">{stat.subLabel}</p>
                            )}
                        </div>
                    </Card>
                ))}
                            ...
                            {isLoading ? (
                                ...
                                >
                                    ...
                                    <td className="px-4 py-5">
                                        ...
                                        {order.hasReturn && (
                                            <div className="mt-1.5 inline-flex items-center gap-1.5 rounded-xl border border-rose-200 bg-rose-50 px-2.5 py-1.5 text-[9px] font-black uppercase tracking-wider text-rose-600">
                                                <RotateCcw className="h-3 w-3" />
                                                <span>{getReturnStatusLabel(order.resolvedReturnStatus)}</span>
                                            </div>
                                        )}
                                    </td>
                                    <td className="px-4 py-5 text-right">
                                        <div className="flex flex-col items-end">
                                            ...
                                            {order.hasReturn && Number(order.returnInfo?.refundAmount || 0) > 0 && (
                                                <span className="text-[10px] font-black text-rose-500 mt-1">
                                                    -₹{Number(order.returnInfo.refundAmount).toLocaleString('en-IN')}{' '}
                                                    {getRefundStatusLabel(order.returnInfo?.refundStatus)}
                                                </span>
                                            )}
                                        </div>
                                    </td>

...
const AdminWallet = () => {
        ...
        try {
            ...
            if (summaryRes.data.success || ledgerRes.data.success) {
                ...
                const mappedTransactions = (ledger.items || []).map((entry) => ({
                    ...
                    status: String(entry.status || "CAPTURED").toUpperCase(),
                ...
                setWalletData({
                    stats: {
                        ...
                        returnPickupFeesPaid: summary.returnPickupFeesPaid || 0,
                        ...
                        returnRefundsToCustomers: summary.returnRefundsToCustomers || 0,
                        returnRefundsAwaitingPayout: summary.returnRefundsAwaitingPayout || 0,
                        returnRefundsRecoveredFromSellers: summary.returnRefundsRecoveredFromSellers || 0,
    ...
    const stats = [
        ...
        {
            label: 'Return Pickup Fees Paid',
            value: `₹${(walletData.stats?.returnPickupFeesPaid || 0).toLocaleString()}`,
            description: 'Rider return pickups funded by admin',
            icon: IndianRupee,
            color: 'rose',
            bg: 'bg-rose-50',
        ...
        },
        {
            ...
            description: 'Owed to sellers (net of return recoveries)',
            icon: CreditCard,
            color: 'blue',
            bg: 'bg-blue-50',
        ...
        },
        {
            ...
            value: `₹${(walletData.stats?.returnRefundsToCustomers || 0).toLocaleString()}`,
            description: `${walletData.stats?.totalReturns || 0} returns · ₹${(walletData.stats?.returnRefundsAwaitingPayout || 0).toLocaleString()} awaiting payout`,
            icon: RotateCcw,
            color: 'rose',
            bg: 'bg-rose-50',
            iconColor: 'text-rose-500'
        ...
        {
            label: 'Recovered From Sellers',
            value: `₹${(walletData.stats?.returnRefundsRecoveredFromSellers || 0).toLocaleString()}`,
            description: 'Return refunds clawed back from seller ledgers',
            icon: RotateCcw,
            color: 'emerald',
            bg: 'bg-emerald-50',
            iconColor: 'text-emerald-500'
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                ...
                <div className="flex items-center space-x-3">
                    ...
                    <button
                        ...
                        className="flex items-center gap-2 px-4 py-2.5 bg-primary text-white rounded-xl text-xs font-bold hover:bg-primary/90 transition-all shadow-lg active:scale-95 disabled:opacity-50"
                    >
                                                        ...
                                                        {req.status === 'PENDING' ? (
                                                            <div className="flex items-center justify-end gap-2">
                                                                <button
                                                                    ...
                                                                    className="px-3 py-1.5 bg-primary text-white rounded-lg text-[10px] font-black uppercase hover:bg-primary/90 transition-all"
                                                                >
                                                                    Approve
                                                                </button>
                                                            </div>
                                            ...
                                            {filteredTransactions.map((txn, i) => (
                                                ...
                                                >
                                                    ...
                                                    <td className="px-6 py-5 text-center">
                                                        <Badge
                                                            variant={
                                                                ['COMPLETED', 'SETTLED', 'CAPTURED', 'PAID'].includes(String(txn.status || '').toUpperCase())
                                                                    ? 'success'
                                                                    : ['FAILED', 'REJECTED', 'REFUNDED'].includes(String(txn.status || '').toUpperCase())
                                                                        ? 'danger'
                                                                        : 'warning'
                                                            }
                                                            className="text-[8px] font-black px-2.5 py-1"
                                                        >
                                                            {String(txn.status || 'PENDING').toUpperCase()}
                                                        </Badge>
                                                    </td>
                                                    <td className="px-6 py-5 text-right pr-8">
                        ...
                        <Card className="p-6 border-none shadow-xl ring-1 ring-slate-100 bg-gradient-to-br from-slate-900 to-slate-800 text-white overflow-hidden relative">
                            <div className="relative z-10 space-y-6">
                                <div>
                                    ...
                                    <h3 className="text-4xl font-black">₹{(walletData.stats?.sellerPendingPayouts || 0).toLocaleString()}</h3>
                                </div>
                                ...
                                <button
                                    onClick={handleProcessPayouts}
                                    disabled={isProcessing}
                                    className="w-full py-4 bg-primary hover:bg-primary/90 text-white rounded-2xl font-black text-[11px] uppercase tracking-widest transition-all shadow-lg active:scale-[0.98] flex items-center justify-center gap-2 group disabled:opacity-50"
                                >
                    ...
                    <div className="space-y-6">
                        ...
                        <div className="pt-4 flex gap-3">
                            <button
                                ...
                                className="flex-1 py-3 bg-primary text-white rounded-xl font-bold text-xs uppercase tracking-widest shadow-xl active:scale-95 transition-all"
                            >
                                CLOSE
                            </button>
                        </div>

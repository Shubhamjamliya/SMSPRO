...
const FAQManagement = () => {
    ...
    const filteredAndSortedFaqs = useMemo(() => {
        ...
        switch (sortBy) {
            ...
            case 'Newest First':
                ...
                break;
            case 'Alphabetical':
                result.sort((a, b) => String(a.question || '').localeCompare(String(b.question || '')));
                break;
            default:
                break;
        }
    ...
    const handleSaveFaq = async (e) => {
        ...
        try {
            ...
            setNewFaq({ question: '', answer: '', category: categories[0]?.name || 'Orders', audience: 'customer', status: 'published' });
        } catch (error) {
    ...
    const handleEditClick = (faq) => {
        setNewFaq({
            ...
            audience: faq.audience || 'customer',
            status: faq.status
        });
    ...
    const handleToggleStatus = async (faq) => {
        try {
            ...
            await adminApi.updateFAQ(faq._id || faq.id, { status: newStatus });
            fetchFaqs(page);
    ...
    const handleAddCategory = async () => {
        ...
        try {
            await adminApi.createFaqCategory({ name: newCategoryName, color: randomColor });
            await fetchCategories();
            setNewCategoryName('');
            showToast('New taxonomy node generated', 'success');
        } catch (error) {
            showToast(error.response?.data?.message || 'Failed to generate category', 'error');
        }
    };
    ...
    const handleDeleteCategory = async (id) => {
        try {
            ...
            showToast(error.response?.data?.message || 'Failed to remove category', 'error');
        }
    };
    ...
    return (
        <div className="ds-section-spacing animate-in fade-in slide-in-from-bottom-4 duration-700 pb-12">
                ...
                <div className="flex items-center gap-3">
                    ...
                    {canCreate && (categories.length === 0 ? (
                        <div className="bg-rose-50 border border-rose-100 rounded-xl p-4 mt-6">
                            <p className="text-sm text-rose-600 font-bold text-center">Please create a category first before adding FAQs.</p>
                        </div>
                    ) : (
                        <button
                            onClick={() => {
                            ...
                            className="flex items-center gap-2 px-5 py-3 bg-primary text-white rounded-2xl text-xs font-bold hover:bg-primary/90 transition-all shadow-lg active:scale-95 shadow-primary/20"
                        >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                {[
                    { label: 'Total FAQs', value: faqs.length, icon: MessageSquare, bg: 'bg-primary/5', iconColor: 'text-primary' },
                    { label: 'Total Views', value: faqs.reduce((acc, f) => acc + Number(f.views || 0), 0).toLocaleString(), icon: TrendingUp, bg: 'bg-sky-50', iconColor: 'text-sky-600' },
                    ...
                    <Card key={i} className="p-5 border-none shadow-sm ring-1 ring-slate-100 bg-white group hover:ring-primary/20 transition-all overflow-hidden relative text-left">
                        ...
                        <div className="space-y-2">
                            <button
                                ...
                                className={cn(
                                    ...
                                    activeCategory === 'All' ? "bg-primary text-white shadow-lg" : "text-slate-600 hover:bg-slate-50"
                                )}
                            >
                            ...
                            {categoriesWithCounts.map((cat) => (
                                <button
                                    ...
                                    className={cn(
                                        "w-full flex items-center justify-between px-4 py-3 rounded-2xl text-xs font-bold transition-all",
                                        activeCategory === cat.name ? "bg-primary text-white shadow-lg shadow-primary/10" : "text-slate-600 hover:bg-slate-50"
                                    )}
                                >
                                    <span className="flex items-center gap-3">
                    ...
                    <div className="space-y-4">
                        <AnimatePresence mode='popLayout'>
                            {filteredAndSortedFaqs.map((faq) => {
                                const faqKey = String(faq.id || faq._id || '');
                                return (
                                <motion.div
                                    key={faqKey}
                                    layout
                                ...
                                >
                                    <Card className={cn(
                                        ...
                                        expandedId === faqKey ? "ring-primary/20 bg-white" : "ring-slate-100 bg-white hover:ring-slate-200"
                                    )}>
                                        <div className="p-6">
                                            <div className="flex items-start gap-4">
                                                ...
                                                <div className="flex-1">
                                                    <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-2">
                                                        <div className="flex items-center gap-3">
                                                            <Badge variant="outline" className="text-[8px] font-black uppercase tracking-widest border-slate-200 text-slate-400">
                                                                {faqKey.substring(0, 8).toUpperCase()}
                                                            </Badge>
                                                        ...
                                                        <div className="flex items-center gap-2">
                                                            <div className="flex items-center gap-2 mr-4">
                                                                ...
                                                                <span className="text-[10px] font-bold text-slate-400">{Number(faq.views || 0).toLocaleString()}</span>
                                                            </div>
                                                            {canEdit && (
                                                                <button
                                                    ...
                                                    <div
                                                        onClick={() => setExpandedId(expandedId === faqKey ? null : faqKey)}
                                                    ...
                                                    >
                                                        <h3 className="text-base font-black text-slate-900 group-hover:text-primary transition-colors flex items-center justify-between">
                                                            {faq.question}
                                                            {expandedId === faqKey ? <ChevronUp className="h-5 w-5" /> : <ChevronDown className="h-5 w-5" />}
                                                        </h3>
                                                    </div>
                                                </div>
                                            </div>
                ...
                <form onSubmit={handleSaveFaq} className="space-y-6 text-left">
                        ...
                        <div className="space-y-4">
                            <div>
                                ...
                                <select
                                    ...
                                    className="w-full px-5 py-3.5 bg-slate-50 border-none rounded-2xl text-sm font-bold outline-none focus:ring-2 focus:ring-primary/10 transition-all shadow-sm cursor-pointer"
                                >
                            ...
                            <div>
                                ...
                                <div className="flex bg-slate-100 p-1 rounded-2xl">
                                    <button
                                        ...
                                        className={cn("flex-1 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all", newFaq.status === 'published' ? "bg-white text-primary shadow-sm" : "text-slate-400")}
                                    >PUBLISHED</button>
                                    <button
                                        type="button"
                                        onClick={() => setNewFaq({ ...newFaq, status: 'draft' })}
                                        className={cn("flex-1 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all", newFaq.status === 'draft' ? "bg-white text-primary shadow-sm" : "text-slate-400")}
                                    >DRAFT</button>
                                </div>
                            </div>
                            <div>
                                <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest block mb-2">Audience</label>
                                <div className="flex bg-slate-100 p-1 rounded-2xl">
                                    <button
                                        type="button"
                                        onClick={() => setNewFaq({ ...newFaq, audience: 'customer' })}
                                        className={cn("flex-1 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all", newFaq.audience === 'customer' ? "bg-white text-primary shadow-sm" : "text-slate-400")}
                                    >CUSTOMER</button>
                                    <button
                                        type="button"
                                        onClick={() => setNewFaq({ ...newFaq, audience: 'seller' })}
                                        className={cn("flex-1 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all", newFaq.audience === 'seller' ? "bg-white text-primary shadow-sm" : "text-slate-400")}
                                    >SELLER</button>
                                </div>
                            </div>
                        </div>
                    ...
                    <div>
                        ...
                        <input
                            ...
                            className="w-full px-5 py-5 bg-slate-50 border-none rounded-2xl text-base font-black outline-none focus:ring-2 focus:ring-primary/10 transition-all shadow-sm"
                        />
                    </div>
                    <div>
                        ...
                        <textarea
                            ...
                            className="w-full px-5 py-5 bg-slate-50 border-none rounded-2xl text-sm font-bold outline-none focus:ring-2 focus:ring-primary/10 transition-all shadow-sm resize-none"
                        />
                    </div>
                    <div className="flex gap-4">
                        ...
                        <button type="submit" className="flex-[2] py-4 bg-primary text-white rounded-2xl font-black text-[11px] uppercase tracking-widest hover:bg-primary/90 shadow-xl shadow-primary/10 transition-all flex items-center justify-center gap-2">
            ...
            >
                <div className="space-y-6 text-left">
                    <div className="space-y-3">
                        {categories.map((cat) => (
                            <div key={cat._id} className="flex items-center justify-between p-4 bg-slate-50 rounded-2xl border border-slate-100">
                                ...
                                <div className="flex items-center gap-2">
                                    {canDelete && (
                                        <button onClick={() => handleDeleteCategory(cat._id)} className="p-2 text-slate-400 hover:text-rose-500 hover:bg-white rounded-lg transition-all"><Trash2 className="h-4 w-4" /></button>
                                    )}
                                </div>
                            </div>
                        ))}
                    ...
                    <button onClick={handleAddCategory} className="w-full py-4 bg-primary text-white rounded-2xl font-black text-[11px] uppercase tracking-widest hover:bg-primary/90 transition-all">GENERATE NEW CATEGORY</button>
                </div>
            </Modal>
        </div>
    );

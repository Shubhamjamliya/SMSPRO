...
const getCouponDisplayStatus = (coupon) => {
    const status = String(coupon?.effectiveStatus || '').trim().toLowerCase();
...
const formatCouponDate = (coupon, which = 'from') => {
    if (which === 'from') {
        if (coupon?.validFromDate) return coupon.validFromDate;
        if (!coupon?.validFrom) return '—';
        return new Intl.DateTimeFormat('en-IN', {
            timeZone: 'Asia/Kolkata',
            day: '2-digit',
            month: 'short',
            year: 'numeric',
        }).format(new Date(coupon.validFrom));
    }
    if (coupon?.validTillDate) return coupon.validTillDate;
    const end = coupon?.validTill || coupon?.expiryDate;
    if (!end) return '—';
    return new Intl.DateTimeFormat('en-IN', {
        timeZone: 'Asia/Kolkata',
        day: '2-digit',
...
const formatCouponDateTime = (value) => {
    if (!value) return '—';
...
const DetailRow = ({ label, value }) => (
    <div className="flex items-start justify-between gap-4 rounded-2xl bg-slate-50 px-4 py-3">
        ...
        <span className="text-xs font-bold text-slate-900 text-right break-words">
            {value ?? '—'}
        </span>
    </div>
);
...
const CouponManagement = () => {
    ...
    const validateForm = () => {
        ...
        if (formData.maxDiscount !== '' && Number(formData.maxDiscount) < 0) return 'Max discount cannot be negative';
        if (formData.usageLimit !== '' && Number(formData.usageLimit) < 0) return 'Usage limit cannot be negative';
        if (formData.perUserLimit !== '' && Number(formData.perUserLimit) < 1) return 'Per user limit must be at least 1';
    ...
    const stats = useMemo(() => {
        ...
        const expiringSoon = coupons.filter((c) => {
            if (!c.validTill || getCouponDisplayStatus(c) !== 'active') return false;
            const till = new Date(c.validTill);
            const diffDays = (till - new Date()) / (1000 * 60 * 60 * 24);
    ...
    const handleOpenModal = (coupon = null) => {
        if (coupon) {
            ...
            setFormData({
                ...
                validFrom: coupon.validFromDate || (coupon.validFrom
                    ...
                    : ''),
                validTill: coupon.validTillDate || (coupon.validTill
                    ? new Intl.DateTimeFormat('en-CA', {
                        timeZone: 'Asia/Kolkata',
                        year: 'numeric',
                        month: '2-digit',
                        day: '2-digit',
                      }).format(new Date(coupon.validTill))
                    : ''),
    ...
    const handleToggleStatus = async (coupon) => {
        try {
            ...
            if (res?.data?.success) {
                ...
                const nextActive = updated ? Boolean(updated.isActive) : !coupon.isActive;
                showToast(`Coupon ${nextActive ? 'activated' : 'deactivated'}`, 'success');
            }
        } catch (error) {
            showToast(error?.response?.data?.message || 'Failed to toggle coupon status', 'error');
        }
    };
            ...
            <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 px-1">
                ...
                {canCreate && (
                    <button
                        ...
                        className="flex items-center gap-2 px-6 py-3.5 bg-primary text-white rounded-2xl text-[10px] font-black uppercase tracking-[0.2em] shadow-xl hover:scale-[1.02] active:scale-95 transition-all"
                    >
                    ...
                    <Card key={i} className="p-6 border-none shadow-xl ring-1 ring-slate-100 bg-white group hover:ring-primary/20 transition-all">
                        <div className="flex items-center justify-between mb-4">
                            <div className={cn("p-2.5 rounded-2xl",
                                s.color === 'indigo' && "bg-primary/5 text-primary",
                                ...
                                <tr key={c._id} className="group hover:bg-slate-50/30 transition-colors">
                                    <td className="px-4 py-6">
                                        <div className="flex items-center gap-4">
                                            ...
                                            <div>
                                                ...
                                                {c.isFirstOrderOnly && (
                                                    <p className="text-[10px] font-bold text-amber-600 mt-1">First order only</p>
                                                )}
                                    ...
                                    <td className="px-4 py-6">
                                        <div className="space-y-1">
                                            ...
                                            {c.discountType === 'percentage' && Number(c.maxDiscount) > 0 && (
                                                <p className="text-[10px] font-bold text-slate-400">Max: ₹{c.maxDiscount}</p>
                                            )}
                                        </div>
                                    </td>
                                    <td className="px-4 py-6">
                                    ...
                                    <td className="px-4 py-6">
                                        <div className="flex items-center gap-2 text-slate-500">
                                            ...
                                            <span className="text-[10px] font-bold uppercase tracking-tighter">
                                                {formatCouponDate(c, 'from')} - {formatCouponDate(c, 'till')}
                                            </span>
                                        </div>
                                    </td>
                                    <td className="px-4 py-6 text-center">
                                        {(() => {
                                            ...
                                            const meta = couponStatusMeta[status] || couponStatusMeta.inactive;
                                            return (
                                                <Badge variant={meta.variant} className="text-[9px] font-black uppercase">
                                                    {meta.label}
                                                </Badge>
                                            );
                                        })()}
                                        ...
                                        <div className="flex items-center justify-end gap-2">
                                            ...
                                            {canEdit && (
                                                <button
                                                    ...
                                                    disabled={getCouponDisplayStatus(c) === 'expired'}
                                                    className={`relative inline-flex h-6 w-12 items-center rounded-full transition-colors ${c.isActive && getCouponDisplayStatus(c) === 'active' ? 'bg-green-500' : 'bg-slate-300'} ${getCouponDisplayStatus(c) === 'expired' ? 'opacity-50 cursor-not-allowed' : ''}`}
                                                    title={getCouponDisplayStatus(c) === 'expired' ? 'Expired coupon' : (c.isActive ? 'Deactivate' : 'Activate')}
                                                >
                                                    <span className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${c.isActive && getCouponDisplayStatus(c) === 'active' ? 'translate-x-7' : 'translate-x-1'}`} />
                                                </button>
                                            )}
                                            {canEdit && (
                                                <button
                        ...
                        >
                            <div className="flex items-start justify-between gap-3 border-b border-slate-100 px-6 py-5">
                                <div>
                                    ...
                                    <h3 className="mt-1 text-lg font-black tracking-wider text-slate-900">
                                        {viewCoupon.code}
                                    </h3>
                                    <div className="mt-2">
                                        {(() => {
                                            const status = getCouponDisplayStatus(viewCoupon);
                                            const meta = couponStatusMeta[status] || couponStatusMeta.inactive;
                                            return (
                                                <Badge variant={meta.variant} className="text-[9px] font-black uppercase">
                                                    {meta.label}
                                ...
                                <DetailRow
                                    ...
                                    value={
                                        viewCoupon.discountType === 'percentage'
                                            ? `${viewCoupon.discountValue}% OFF`
                                            : `₹${viewCoupon.discountValue} OFF`
                                    }
                                />
                                <DetailRow
                                    label="Discount type"
                                    value={viewCoupon.discountType === 'fixed' ? 'Fixed amount' : 'Percentage'}
                                />
                                <DetailRow
                                    label="Min order"
                                    value={
                                        Number(viewCoupon.minOrderValue) > 0
                                            ? `₹${viewCoupon.minOrderValue}`
                                            : 'No minimum'
                                    }
                                />
                                <DetailRow
                                    label="Max discount"
                                    value={
                                        Number(viewCoupon.maxDiscount) > 0
                                            ? `₹${viewCoupon.maxDiscount}`
                                            : 'No cap'
                                    }
                                />
                                <DetailRow
                                    label="Usage limit"
                                    value={
                                        Number(viewCoupon.usageLimit) > 0
                                            ? `${viewCoupon.usedCount || 0} / ${viewCoupon.usageLimit}`
                                            : `${viewCoupon.usedCount || 0} used · Unlimited`
                                    }
                                />
                                <DetailRow
                                    label="Per user limit"
                                    value={Number(viewCoupon.perUserLimit) > 0 ? viewCoupon.perUserLimit : '1'}
                                />
                                <DetailRow
                                    label="First order only"
                                    value={viewCoupon.isFirstOrderOnly ? 'Yes' : 'No'}
                                />
                                <DetailRow label="Valid from" value={formatCouponDate(viewCoupon, 'from')} />
                                <DetailRow label="Valid till" value={formatCouponDate(viewCoupon, 'till')} />
                                <DetailRow
                                    label="Description"
                                    value={viewCoupon.description || '—'}
                                />
                                <DetailRow
                                    label="Created"
                                    value={formatCouponDateTime(viewCoupon.createdAt)}
                                />
                                <DetailRow
                                    label="Updated"
                                    value={formatCouponDateTime(viewCoupon.updatedAt)}
                                />
                                {viewCoupon._id ? (
                                    <DetailRow label="Coupon ID" value={String(viewCoupon._id)} />
                                ) : null}
                            </div>
                            ...
                            <div className="flex gap-3 border-t border-slate-100 px-6 py-4">
                                <button
                                    type="button"
                                    onClick={() => setViewCoupon(null)}
                                    className="flex-1 rounded-2xl bg-slate-100 py-3 text-[10px] font-black uppercase tracking-widest text-slate-500"
                                >
                                    Close
                                </button>
                                {canEdit && (
                                    <button
                                        type="button"
                                        onClick={() => {
                                            const coupon = viewCoupon;
                                            setViewCoupon(null);
                                            handleOpenModal(coupon);
                                        }}
                                        className="flex-1 rounded-2xl bg-primary py-3 text-[10px] font-black uppercase tracking-widest text-white"
                                    >
                                        Edit coupon
                                    </button>
                                )}
            ...
            >
                <form onSubmit={handleSubmit} className="space-y-6">
                    ...
                    <label className="flex items-center gap-3 rounded-2xl bg-slate-50 px-4 py-3 cursor-pointer">
                        ...
                        />
                        <span className="text-xs font-bold text-slate-700">First order only</span>
                    </label>
                    <p className="text-[10px] text-slate-400 -mt-3">
                        Same start and end date is allowed for single-day coupons.
                    </p>

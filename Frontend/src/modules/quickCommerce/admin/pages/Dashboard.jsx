...
const AdminDashboard = () => {
    ...
    const stats = [
        ...
        {
            label: 'Returns',
            value: overview.totalReturns?.toLocaleString() || '0',
            icon: RotateCcw,
            color: 'text-rose-600',
            bg: 'bg-rose-50',
            trend: `${overview.activeReturns || 0} active`,
            trendDirection: 'down',
            description: 'Return requests raised'
        },
        {
            label: 'Refunded To Customers',
            value: `₹${overview.refundedToCustomers?.toLocaleString() || '0'}`,
            icon: RotateCcw,
            color: 'text-amber-600',
            bg: 'bg-amber-50',
            trend: `₹${(overview.refundsPending || 0).toLocaleString()} pending`,
            trendDirection: 'down',
            description: 'Completed return refunds'
        },
    ];
                        ...
                        {isDropdownOpen && (
                            <div className="absolute right-0 mt-2 w-52 bg-white rounded-xl shadow-xl border border-slate-100 py-1.5 z-50 animate-in fade-in slide-in-from-top-2">
                                ...
                                <button
                                    ...
                                    className="w-full text-left px-4 py-2.5 text-sm font-semibold text-slate-700 hover:bg-primary/5 transition-colors flex items-center gap-3 disabled:opacity-60"
                                >

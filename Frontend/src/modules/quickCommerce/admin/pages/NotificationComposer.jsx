...
export default function NotificationComposer() {
  ...
  const loadHistory = async () => {
    try {
      ...
      setHistory(response?.data?.result?.items || response?.data?.data?.items || []);
    } catch {
      setHistory([]);
    } finally {
  ...
  const loadRecipients = async () => {
    try {
      ...
      const [customersRes, sellersRes, deliveryRes] = await Promise.all([
        foodAdminAPI.getCustomers({ page: 1, limit: 500 }),
        adminApi.getSellers(),
        foodAdminAPI.getDeliveryPartners({ page: 1, limit: 500 }),
      ]);
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      setForm({ title: "", message: "", targetType: "ALL_QC" });
      setSelectedRecipients([]);
      setSearch("");
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <label className="block">
            ...
            <textarea
              ...
              rows={5}
              className="mt-2 w-full rounded-2xl border border-slate-200 px-4 py-3 text-sm outline-none focus:border-primary focus:ring-2 focus:ring-primary/10 resize-y"
            />
          </label>
          ...
          <div className="flex justify-end">
            <button
              type="submit"
              disabled={submitting}
              className="inline-flex items-center gap-2 rounded-2xl bg-primary px-5 py-3 text-sm font-semibold text-white hover:bg-primary/90 disabled:opacity-60"
            >
                  ...
                  <tr key={item?._id} className="border-b border-slate-100 align-top">
                    ...
                    <td className="py-4 text-right">
                      <button
                        ...
                        className="inline-flex items-center gap-2 rounded-xl border border-primary/20 px-3 py-2 text-xs font-semibold text-primary hover:bg-primary/5"
                      >

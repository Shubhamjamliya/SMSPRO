...
const resolveSellerRecovery = (row) =>
  Number(row?.finance?.preSettlementDeducted || 0) +
  Number(row?.finance?.postSettlementDebited || 0);
...
const AdminReturns = () => {
  ...
  const handleExport = () => {
    ...
    const rows = [
      ...
      ...filteredReturns.map((row) => [
        ...
        resolveSellerRecovery(row).toFixed(2),
        row.finance?.settlementMode || "",
        row.customer?.name || "",
        row.sellerId,
  ...
  const adminPickupFees = financeReport?.adminPickupFeeExpenses || null;
  const adminPickupFeeEntries = adminPickupFees?.entries || [];
  ...
  return (
    <div className="space-y-6 pb-16">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-xs font-bold uppercase tracking-widest text-slate-500">
                Return pickup fees paid
              </p>
              <p className="text-2xl font-black text-slate-900">
                ₹{Number(adminPickupFees?.totalPaid || 0).toFixed(2)}
              </p>
              <p className="text-[11px] text-slate-500">
                Admin-funded · {adminPickupFees?.count || 0} pickup
                {(adminPickupFees?.count || 0) === 1 ? "" : "s"}
              </p>
            </div>
          </div>
        </Card>
      ...
      {adminPickupFeeEntries.length > 0 && (
        <Card className="border-none shadow-sm ring-1 ring-slate-100 overflow-hidden">
          <div className="p-4 border-b border-slate-100 bg-slate-50/50">
            <p className="text-sm font-black text-slate-900">Admin return pickup expenses</p>
            <p className="text-xs text-slate-500">
              Rider return pickup fees funded by the platform. Sellers are not debited for these.
            </p>
          </div>
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-slate-100">
                {adminPickupFeeEntries.slice(0, 20).map((entry) => (
                  <tr key={entry.transactionId} className="text-slate-700">
                    <td className="px-4 py-2 font-semibold">{entry.orderId || "—"}</td>
                    <td className="px-4 py-2 font-mono text-xs">
                      {entry.returnId ? entry.returnId.slice(-8).toUpperCase() : "—"}
                    </td>
                    <td className="px-4 py-2 font-mono text-xs">
                      {entry.sellerId ? entry.sellerId.slice(-6) : "—"}
                    </td>
                    <td className="px-4 py-2 font-mono text-xs">
                      {entry.riderId ? entry.riderId.slice(-6) : "—"}
                    </td>
            ...
            <table className="w-full text-sm">
              <thead className="bg-slate-50 text-left text-xs uppercase tracking-widest text-slate-500">
                <tr>
                  ...
                  <th className="px-4 py-3">Recovered from seller</th>
                    ...
                    <td className="px-4 py-3">
                      ...
                      {row.finance?.settlementMode ? (
                        <p className="text-[10px] text-slate-500 capitalize">
                          {row.finance.settlementMode.replace(/_/g, " ")}
                        </p>
                      ) : null}
                    </td>
                    <td className="px-4 py-3 text-xs text-slate-500">
                      {row.returnRequestedAt
                        ? new Date(row.returnRequestedAt).toLocaleDateString("en-IN", {
                            day: "numeric",
                            month: "short",
                            year: "numeric",
                          })
                ...
                <>
                  ...
                  <div className="rounded-2xl border border-slate-100 bg-white p-4 space-y-1 text-xs">
                    ...
                    {selectedReturn.finance ? (
                      <>
                        ...
                        <p>Seller pickup fee debit: ₹{Number(selectedReturn.finance.pickupFeeDebited || 0).toFixed(2)}</p>
                        <p>
                          Admin-funded pickup fee: ₹
                          {Number(selectedReturn.finance.pickupFeeAdminExpense || 0).toFixed(2)}
                          {selectedReturn.finance.pickupFeeFundedBy
                            ? ` (${selectedReturn.finance.pickupFeeFundedBy})`
                            : ""}
                        </p>
                  ...
                  {Array.isArray(selectedReturn.returnItems) && selectedReturn.returnItems.length > 0 && (
                    <div className="space-y-2">
                      ...
                      <ReturnItemsList items={selectedReturn.returnItems} />
                    </div>
                  )}
                </>
              )}
              ...
              <div className="flex flex-wrap gap-2">
                ...
                {selectedReturn.refundStatus === "pending" && (
                  <button
                    ...
                    className="inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-primary text-white text-xs font-bold disabled:opacity-60"
                  >

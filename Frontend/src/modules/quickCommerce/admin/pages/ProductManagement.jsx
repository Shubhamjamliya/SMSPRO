...
const ProductManagement = () => {
    ...
    const modalTabs = useMemo(() => {
        const tabs = [
            ...
            { id: 'variants', label: 'Variants & Packing', icon: HiOutlineSwatch },
    ...
    const handleSave = async () => {
        ...
        if (!formData.name || !formData.header || !formData.categoryId) {
            return toast.error('Please fill product title and categories');
        }
    ...
    return (
        <div className="ds-section-spacing animate-in fade-in slide-in-from-bottom-2 duration-700 pb-16">
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                {[
                    { label: 'All Items', val: stats.total, icon: HiOutlineCube, color: 'text-primary', bg: 'bg-primary/5' },
                        ...
                        <tbody className="divide-y divide-slate-50">
                            {isLoading ? (
                                ...
                                <tr key={p._id} className="hover:bg-slate-50/30 transition-colors group">
                                    ...
                                    <td className="px-6 py-4">
                                        <div className="flex items-center gap-3">
                                            <div className="h-10 w-10 rounded-lg overflow-hidden bg-slate-100 ring-1 ring-slate-200">
                                                <img src={p.mainImage || p.variants?.[0]?.images?.[0] || p.images?.[0]} alt={p.name} className="h-full w-full object-cover group-hover:scale-110 transition-transform duration-500" />
                                            </div>
                                            <div>
                                    ...
                                    {/* Subcategory Column */}
                                    <td className="px-6 py-4">
                                        <span className="text-xs font-bold text-slate-600">{p.categoryId?.name || 'N/A'}</span>
                                    </td>
                            ...
                            <div className="flex flex-col lg:flex-row flex-1 min-h-[400px] max-h-[calc(100vh-200px)] overflow-hidden">
                                ...
                                <div className="lg:w-1/4 bg-slate-50/50 border-r border-slate-100 p-4 space-y-1 overflow-y-auto scrollbar-hide">
                                    ...
                                    <div className="pt-8 px-4">
                                        ...
                                        <div className="mt-3 p-4 bg-primary/5 rounded-2xl border border-primary/15 flex items-center justify-between">
                                                        ...
                                                        <div key={idx} className="w-32 aspect-square rounded-2xl border-2 border-slate-200 bg-slate-50 relative overflow-hidden group">
                                                            ...
                                                            {!isViewMode && (
                                                                <button
                                                                    ...
                                                                    className="absolute top-2 right-2 p-1.5 bg-white/90 text-red-500 rounded-lg opacity-0 group-hover:opacity-100 transition-opacity hover:bg-primary/5"
                                                                >
                            ...
                            <div className="p-6 border-t border-slate-100 bg-slate-50/50 flex items-center justify-end gap-3">
                                ...
                                {!isViewMode && (
                                    <button
                                        onClick={handleSave}
                                        disabled={isSaving}
                                        className="bg-primary text-white px-10 py-2.5 rounded-xl text-xs font-bold shadow-xl hover:-translate-y-0.5 transition-all disabled:opacity-50"
                                    >
                ...
                <div className="py-2">
                    ...
                    <div className="mt-8 flex justify-end">
                        <button
                            ...
                            className="bg-primary text-white px-8 py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest shadow-xl hover:-translate-y-0.5 transition-all active:scale-95"
                        >
                            CLOSE VIEWER
                        </button>
                    </div>

  ...
  const fetchZone = async () => {
    try {
      ...
      if (response.data?.success && response.data.data?.zone) {
        ...
        setAdminHub(response.data.data.adminHub || null)
      }
    } catch (error) {
          ...
          <div className="lg:col-span-1">
            <div className="bg-white rounded-lg shadow-sm border border-slate-200 p-6">
              ...
              <div className="space-y-4">
                ...
                <div>
                  <label className="block text-sm font-semibold text-slate-700 mb-1">Zone Type</label>
                  <span className={`inline-block px-2 py-1 rounded-full text-xs font-medium ${
                    zone.zoneType === "single_vendor"
                      ? "bg-violet-100 text-violet-800"
                      : "bg-sky-100 text-sky-800"
                  }`}>
                    {zone.zoneType === "single_vendor" ? "Single Vendor" : "Multi Vendor"}
                  </span>
                </div>

                <div>
                  <label className="block text-sm font-semibold text-slate-700 mb-1">Admin Hub</label>
                  <p className="text-sm text-slate-900">
                    {zone.adminHubEnabled || zone.adminHubSellerId || zone.zoneType === "single_vendor"
                      ? "Enabled"
                      : "Disabled"}
                  </p>
                  {adminHub && (
                    <p className="text-xs text-slate-600 mt-1">
                      {adminHub.shopName || adminHub.name}
                      {adminHub.phoneMasked ? ` · ${adminHub.phoneMasked}` : ""}
                      {adminHub.approvalStatus ? ` · ${adminHub.approvalStatus}` : ""}
                    </p>
                  )}
                </div>

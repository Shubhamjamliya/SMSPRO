...
import { isTaxiActiveOrder } from '@/modules/DeliveryV2/utils/taxiRideFlow';

/** Teardrop map pin (data-URL) for pickup / drop */
function buildMapPinIcon({ fill = '#10B981', letter = '', size = 52 } = {}) {
  ...
  const svg = `
    <svg xmlns="http://www.w3.org/2000/svg" width="${size}" height="${size}" viewBox="0 0 48 56">
      ...
      <path filter="url(#pinShadow)" fill="${fill}" d="M24 2C14.06 2 6 10.06 6 20c0 12.5 18 32 18 32s18-19.5 18-32C42 10.06 33.94 2 24 2z"/>
...
export const LiveMap = ({
  ...
  const taxiPickupStop = useMemo(() => {
    if (!isTaxiTrip || !activeOrder) return null;
    return (
      normalizeLocationPoint(activeOrder.restaurantLocation) ||
      normalizeLocationPoint(activeOrder.pickup) ||
      pickupStops[0]?.location ||
      null
    );
  }, [isTaxiTrip, activeOrder, pickupStops]);
  const taxiDropStop = useMemo(() => {
    if (!isTaxiTrip || !activeOrder) return null;
    return (
      normalizeLocationPoint(activeOrder.customerLocation) ||
      normalizeLocationPoint(activeOrder.drop) ||
      customerStop ||
      null
    );
  }, [isTaxiTrip, activeOrder, customerStop]);

  const taxiPickupIcon = useMemo(
    () => (isTaxiTrip && isLoaded ? buildMapPinIcon({ fill: '#10B981', letter: 'P', size: 52 }) : null),
    [isTaxiTrip, isLoaded],
  );
  const taxiDropIcon = useMemo(
    () => (isTaxiTrip && isLoaded ? buildMapPinIcon({ fill: '#EF4444', letter: 'D', size: 52 }) : null),
    [isTaxiTrip, isLoaded],
  );
  ...
  return (
    <div className="absolute inset-0 z-0 text-gray-900 overflow-hidden flex flex-col">
      ...
      >
        ...
        {remainingPath.length > 0 && (
          <>
            ...
            <Polyline
              path={remainingPath}
              options={{
                strokeColor: isTaxiTrip ? '#0A0A0A' : '#22c55e',
                strokeOpacity: 0.95,
                strokeWeight: isTaxiTrip ? 5 : 6,
                zIndex: 10,
              }}
            />
          </>
        ...
        {isTaxiTrip ? (
          <>
            {taxiPickupStop && taxiPickupIcon ? (
              <Marker position={taxiPickupStop} icon={taxiPickupIcon} zIndex={4} />
            ) : null}
            {taxiDropStop && taxiDropIcon ? (
              <Marker position={taxiDropStop} icon={taxiDropIcon} zIndex={4} />
            ) : null}
          </>
        ) : (
          <>

...
const PORTER_OTP_LENGTH = 6;

const PorterCustomerOtpInput = ({ otp, onChange }) => {
  const inputRefs = React.useRef([]);
  ...
  const handleOtpChange = (index, value) => {
    ...
    next[index] = value.substring(value.length - 1);
    onChange(next);
    if (value && index < PORTER_OTP_LENGTH - 1) {
      inputRefs.current[index + 1]?.focus();
    }
  };
    ...
    <div className="flex justify-center gap-2 flex-wrap">
      {otp.map((digit, index) => (
        <input
          ...
          onChange={(e) => handleOtpChange(index, e.target.value)}
          onKeyDown={(e) => handleKeyDown(index, e)}
          className="w-10 h-12 rounded-xl border-2 border-[#2F6BFF]/40 bg-white text-center text-xl font-black text-gray-900 focus:border-[#2F6BFF] outline-none shadow-sm"
        />
      ))}
    </div>
  );
...
const PorterPickupVerification = ({
  customerOtpDigits,
  setCustomerOtpDigits,
}) => {
  const customerOtp = customerOtpDigits.join('');
  return (
    <div className="rounded-xl border border-[#2F6BFF]/25 bg-[#2F6BFF]/5 p-3 space-y-3">
      <div className="min-w-0">
        <p className="text-[9px] font-bold uppercase tracking-widest text-[#2F6BFF]">
          Parcel pickup OTP
        </p>
        <p className="text-xs font-semibold text-gray-700 leading-snug">
      ...
      <PorterCustomerOtpInput
        otp={customerOtpDigits}
        onChange={setCustomerOtpDigits}
      />
      <p className={`text-center text-[9px] font-bold uppercase tracking-widest ${
        customerOtp.length >= PORTER_OTP_LENGTH ? 'text-green-600' : 'text-gray-400'
      }`}>
        {customerOtp.length >= PORTER_OTP_LENGTH ? '✓ OTP ready' : `${customerOtp.length}/${PORTER_OTP_LENGTH} digits`}
      </p>
    </div>
  );
};
...
export const RenderPickupVerification = (props) => {
  ...
  const moduleType = String(order?.module || order?.jobType || order?.orderType || order?.serviceType || order?.type || '').toLowerCase();
  
  if (isReturnPickup) {
    return <ReturnPickupVerification {...props} />;
  }

  if (moduleType === 'porter' || moduleType === 'parcel') {
    return <PorterPickupVerification {...props} />;
  }

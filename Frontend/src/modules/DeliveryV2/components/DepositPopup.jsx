...
export default function DepositPopup({ onSuccess, cashInHand = 0 }) {
  const [step, setStep] = useState(1) // 1: Amount Summary, 2: Payment Mode Selection, 3: Direct Admin details
  const [paymentMode, setPaymentMode] = useState("") // "online" | "admin"
  ...
  // manual upload / admin settlement flow
  const handleManualSubmission = async (depositType) => {
    ...
    try {
      ...
      if (res?.data?.success) {
        toast.success("Deposit proof submitted successfully! Awaiting Admin approval.")
        window.dispatchEvent(new CustomEvent("deliveryWalletStateUpdated"))
        if (onSuccess) onSuccess()
      } else {
  ...
  return (
    <div className="flex flex-col bg-white overflow-hidden h-full w-full text-slate-900">
      ...
      {/* STEP 3: ADMIN DETAILS SETTLEMENT */}
      {step === 3 && paymentMode === "admin" && (

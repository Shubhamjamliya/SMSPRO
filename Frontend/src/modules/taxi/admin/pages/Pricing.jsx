...
const emptyVehicleRates = () => ({
  ...
  baseDistanceKm: 0,
  perKmRate: 12,
  perMinRate: 0, // trip time not billed — kept for API shape
  freeWaitMinutes: 5,
  perMinWaitRate: 2,
  platformFee: 5,
  surgeMultiplier: 1,
...
const buildVehiclesMap = (vehicleTypes = []) => {
  const vehicles = {};
  vehicleTypes.forEach((v) => {
    if (v?.id) vehicles[v.id] = emptyVehicleRates();
  });
  return vehicles;
};

const emptySlab = (fromKm = 0, toKm = 2, vehicleTypes = []) => ({
  fromKm,
  toKm,
  vehicles: buildVehiclesMap(vehicleTypes),
});
...
const rateFields = [
  ...
  { key: "freeWaitMinutes", label: "Free wait" },
  { key: "perMinWaitRate", label: "Wait/min" },
...
const Pricing = () => {
  ...
  const buildGroupSlabs = useCallback((group) => {
    ...
    return templateSlabs.map((range) => {
      ...
      group.rows.forEach((row) => {
        const vid = row.vehicleTypeId || row.vehicleType?.id;
        if (!vid) return;
        const match =
          (row.slabs || []).find(
        ...
        const name =
          ...
          vehicleTypes.find((v) => v.id === vid)?.name ||
          "Vehicle";
        vehicles[vid] = {
          name,
          ...
          perMinRate: Number(match.perMinRate || 0),
          freeWaitMinutes: Number(match.freeWaitMinutes || 0),
          perMinWaitRate: Number(match.perMinWaitRate || 0),
  ...
  const openCreate = () => {
    if (!vehicleTypes.length) {
      toast.error("Add taxi vehicle types in Global Settings first");
      return;
    }
    ...
    setForm({
      zoneId: "",
      status: "active",
      slabs: [emptySlab(0, 2, vehicleTypes)],
      vehicleCommissions,
    });
    setModalOpen(true);
  };
  ...
  const openEdit = (group) => {
    ...
    const slabs = templateSlabs.map((range) => {
      const vehicles = buildVehiclesMap(vehicleTypes);
      group.rows.forEach((row) => {
        const vid = row.vehicleTypeId || row.vehicleType?.id;
        if (!vid) return;
        const match =
    ...
    vehicleTypes.forEach((v) => {
      const row = group.rows.find(
        (r) => String(r.vehicleTypeId || r.vehicleType?.id) === String(v.id),
      );
  ...
  const updateVehicleRate = (slabIndex, vehicleTypeId, patch) => {
    setForm((f) => ({
      ...f,
      slabs: f.slabs.map((s, i) => {
        ...
        return {
          ...
          vehicles: {
            ...s.vehicles,
            [vehicleTypeId]: {
              ...(s.vehicles?.[vehicleTypeId] || emptyVehicleRates()),
              ...patch,
            },
          },
        };
  ...
  const addSlab = () => {
    setForm((f) => {
      ...
      return {
        ...f,
        slabs: [...slabs, { fromKm: prevTo, toKm: prevTo + 5, vehicles }],
      };
    });
  };
  ...
  const save = async () => {
    if (!vehicleTypes.length) {
      toast.error("No vehicle types configured");
      return;
    }
    ...
    const slabs = form.slabs.map((s) => {
      ...
      vehicleTypeIds.forEach((id) => {
        ...
        vehicles[id] = {
          ...
          perMinWaitRate: Number(r.perMinWaitRate || 0),
    ...
    setSaving(true);
    try {
      await taxiAdminApi.saveZonePricingMatrix({
      ...
      toast.success("Zone pricing saved for all vehicle types");
      setModalOpen(false);
      await load();
    } catch (err) {
  ...
  const confirmDelete = async () => {
    ...
    setDeleting(true);
    try {
      await taxiAdminApi.deleteZonePricingMatrix(deleteTarget.zoneId);
    ...
    },
    {
      ...
      cell: (row) => (
        <span className="text-sm text-gray-700">
          {row.vehicleCount} type{row.vehicleCount === 1 ? "" : "s"}
        </span>
      ),
    },
    {
  ...
  return (
    <div className="just-order-theme-scope space-y-6 max-w-[90rem] mx-auto px-4 sm:px-6 lg:px-8 py-8 pb-24">
      <PageHeader
        ...
        description="Pick a zone, define distance slabs, then set rates for every vehicle type in each slab."
        actions={
      ...
      {!vehicleTypes.length && !loading && (
        <p className="text-sm text-amber-700 bg-amber-50 border border-amber-100 rounded-xl px-4 py-3">
          Configure taxi vehicle types in Global Settings before adding pricing.
        </p>
      )}
        ...
        <div className="p-4">
          <AdminTable
            ...
            emptyState={{
              ...
              description: "Add a zone matrix with distance slabs and rates for all vehicles.",
            }}
          />
        </div>
      </SectionCard>
        ...
        <DialogContent className="sm:max-w-[920px]">
          ...
          <div className="space-y-4 max-h-[75vh] overflow-y-auto">
            <p className="rounded-xl border border-orange-100 bg-orange-50/70 px-3 py-2 text-xs text-gray-700">
              For each distance slab (e.g. 0–2 km), set prices for <strong>all vehicle types</strong> at once.
              The matching slab applies to the whole trip.
            </p>
            ...
            <div className="rounded-2xl border border-gray-200 bg-white p-3 space-y-3">
              <div>
                ...
                <p className="text-[11px] text-muted-foreground mt-0.5">
                  Taken from the ride amount after platform fee. Example: total ₹60, fee ₹0, 10% → admin ₹6, driver ₹54.
                </p>
              </div>
        ...
        <DialogContent className="sm:max-w-[920px]">
          ...
          <div className="space-y-4 max-h-[75vh] overflow-y-auto">
            <div className="flex flex-wrap items-center gap-2 text-xs text-gray-600">
              ...
              />
              <span>
                {viewTarget?.vehicleCount || 0} vehicle type
                            ...
                            <tr key={vid} className="border-t border-gray-100">
                              ...
                              {rateFields.map((f) => (
                                <td key={f.key} className="px-3 py-2.5 text-gray-800 whitespace-nowrap">
                                  {f.key === "baseDistanceKm" || f.key === "freeWaitMinutes"
      ...
      >
        <DialogContent className="sm:max-w-[420px]">
          ...
          <div className="text-sm text-gray-600">
            ...
            (all vehicle types in this zone)?
          </div>
          <div className="flex justify-end gap-2">

...
import { Bike, ChevronLeft, ChevronRight, LogOut, X } from "lucide-react";
import { vendorSidebarMenu } from "../utils/vendorSidebarMenu";
import { clearBikeVendorAuth, getBikeVendorUser } from "../utils/authVendor";

const SIDEBAR_STATE_KEY = "bike_rent_vendor_sidebar_state";
...
export default function VendorSidebar({ isOpen = false, onClose, onCollapseChange }) {
  ...
  return (
    <div
      className={`vendor-sidebar-scroll-host backdrop-blur-md border-r border-[#EDE8E0] h-screen fixed left-0 top-0 z-100 flex flex-col overflow-hidden shadow-xs transform transition-all duration-300 ease-in-out lg:translate-x-0 ${
        isOpen ? "translate-x-0" : "-translate-x-full"
      } ${isCollapsed ? "w-20" : "w-80"}`}
      style={{ backgroundColor: "#ffffffcc" }}
    >
      <style>{`
        .vendor-sidebar-scroll::-webkit-scrollbar { width: 2px; }
        .vendor-sidebar-scroll::-webkit-scrollbar-track { background: transparent; }
        .vendor-sidebar-scroll::-webkit-scrollbar-thumb { background: rgba(0, 0, 0, 0.1); border-radius: 10px; }
        .vendor-sidebar-scroll:hover::-webkit-scrollbar { width: 6px; }
        .vendor-sidebar-scroll { scrollbar-width: thin; scrollbar-color: rgba(0, 0, 0, 0.15) transparent; }
      `}</style>
      ...
      <nav className="vendor-sidebar-scroll flex-1 min-h-0 space-y-2 overflow-y-auto overscroll-y-contain px-3 py-3">
        {vendorSidebarMenu.map((section, index) => (

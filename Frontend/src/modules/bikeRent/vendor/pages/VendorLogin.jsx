...
import { ArrowLeft, ArrowRight, Bike, KeyRound, Loader2 } from "lucide-react";
...
export default function VendorLogin() {
  ...
  const requestOtp = async () => {
    ...
    setLoading(true);
    try {
      const data = await bikeVendorApi.requestOtp(digits);
      setPhone(digits);
  ...
  const verifyOtp = async () => {
    ...
    setLoading(true);
    try {
      const data = await bikeVendorApi.verifyOtp({ phone, otp: code });

      // Defensive: guarantee no stale session/data survives if a different
      // vendor had previously logged in on this same browser.
      clearBikeVendorAuth();
      ...
      if (data?.needsRegistration) {
        ...
        setBikeVendorPendingPhone(phone);
        toast.message("Complete your vendor registration");
        navigate("/bike-rent/vendor/onboarding", {
          state: { phone, resumeStep: data.resumeStep || 1, vendor: data.vendor || null },
        });
        return;
      }
      ...
      if (data?.isPending) {
        ...
        setBikeVendorPendingPhone(phone);
        navigate("/bike-rent/vendor/pending");
        return;
      }
      ...
      if (data?.isRejected) {
        ...
        toast.error(data.rejectionReason || "Registration was rejected. Please resubmit.");
        navigate("/bike-rent/vendor/onboarding", {
          state: { phone, rejected: true, rejectionReason: data.rejectionReason, vendor: data.vendor },
        });
        return;
      }
      ...
      if (data?.accessToken && data?.vendor) {
        setBikeVendorAuth(data.accessToken, data.vendor, data.refreshToken);
        navigate("/bike-rent/vendor/dashboard");
        return;
      }
    ...
    <div className="min-h-screen bg-[radial-gradient(circle_at_top,_#fff4ec,_#f7f7f8_45%)] px-4 py-8">
      <div className="mx-auto w-full max-w-md">
          ...
          {step === "phone" ? (
            <div className="space-y-4">
              ...
              <div className="flex overflow-hidden rounded-2xl border border-gray-200 focus-within:border-[#FF6A00] focus-within:ring-2 focus-within:ring-[#FF6A00]/20">
                <span className="bg-gray-50 px-3 py-3 text-sm font-semibold text-gray-600">
                  {COUNTRY}
                </span>
                <input
                  type="tel"
            ...
            <div className="space-y-4">
              ...
              {debugOtp ? (
                <p className="text-center text-xs text-gray-400">Dev OTP: {debugOtp}</p>
              ) : null}
        ...
        <p className="mt-6 text-center text-xs text-gray-400">
          Same mobile number uses your shared Appzeto account & wallet across modules.
        </p>
      </div>
    </div>
  );

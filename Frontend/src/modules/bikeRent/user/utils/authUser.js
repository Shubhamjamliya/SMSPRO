...
export const BIKE_RENT_USER_LOGIN_PATH = "/user/auth/login";

export function isBikeRentUserLoggedIn() {
  return isModuleAuthenticated("user");
}

/** Navigate to shared user login and return here after success. */
export function redirectToBikeRentLogin(navigate, locationOrPath) {
  const state =
    typeof locationOrPath === "string"
      ? buildLoginRedirectState({ pathname: locationOrPath })
      : buildLoginRedirectState(locationOrPath);
  navigate(BIKE_RENT_USER_LOGIN_PATH, { state, replace: false });
}
...
export function getProfileDrivingLicense(user) {
  if (!user) return "";
  return String(

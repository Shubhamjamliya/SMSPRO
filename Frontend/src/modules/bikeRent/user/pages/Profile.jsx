...
import {
  ...
  getBikeRentDeleteAccountPath,
  getBikeRentEditProfilePath,
  getBikeRentHomePath,
  getBikeRentNotificationsPath,
  getBikeRentPrivacyPath,
  getBikeRentProfileContactPath,
  getBikeRentProfileFaqsPath,
  getBikeRentProfileHelpPath,
  getBikeRentRefundPath,
  getBikeRentSecurityPath,
  getBikeRentTermsPath,
  getBikeRentWalletPath,
} from "../utils/routes";
import {
  redirectToBikeRentLogin,
  BIKE_RENT_USER_LOGIN_PATH,
} from "../utils/authUser";
...
const MENU = [
  {
    group: "Account",
    items: [
      { id: "profile", title: "My Profile", icon: User, path: getBikeRentEditProfilePath, auth: true },
      { id: "addresses", title: "Saved Addresses", icon: MapPinned, path: getBikeRentAddressesPath, auth: true },
      { id: "wallet", title: "My Wallet", icon: Wallet, path: getBikeRentWalletPath, auth: true },
    ],
  },
  {
    group: "Preferences",
    items: [
      { id: "notifications", title: "Notifications", icon: Bell, path: getBikeRentNotificationsPath, auth: true },
      { id: "security", title: "Security", icon: Shield, path: getBikeRentSecurityPath, auth: true },
    ],
  },
  {
    group: "Support & legal",
    items: [
      { id: "faqs", title: "FAQs", icon: CircleHelp, path: getBikeRentProfileFaqsPath },
      { id: "contact", title: "Contact Support", icon: Phone, path: getBikeRentProfileContactPath },
      { id: "help", title: "Help Center", icon: BookOpen, path: getBikeRentProfileHelpPath },
      { id: "privacy", title: "Privacy Policy", icon: FileText, path: getBikeRentPrivacyPath },
      { id: "terms", title: "Terms & Conditions", icon: FileText, path: getBikeRentTermsPath },
      { id: "refund", title: "Refund Policy", icon: FileText, path: getBikeRentRefundPath },
    ],
  },
];

export default function BikeRentProfile() {
  ...
  const goAuthRequired = (path) => {
    if (!isLoggedIn) {
      ...
      redirectToBikeRentLogin(navigate, path || location);
      return;
    }
    navigate(path);
  };
  ...
  const handleLogout = async () => {
    ...
    try {
      ...
      navigate(BIKE_RENT_USER_LOGIN_PATH, { replace: true });
    } catch {
      clearModuleAuth("user");
      window.dispatchEvent(new Event("userAuthChanged"));
      setLogoutOpen(false);
      ...
      <main className="space-y-5 px-4 py-4">
          ...
          {loading ? (
            ...
            <>
              ...
              <PrimaryButton
                ...
                variant="outline"
                onClick={() => goAuthRequired(getBikeRentEditProfilePath())}
              >
                Edit Profile
              </PrimaryButton>
            </>
            ...
            <>
              ...
              <PrimaryButton
                className="mt-3.5 h-10"
                onClick={() => redirectToBikeRentLogin(navigate, location)}
              >
        ...
        <section className="space-y-2">
          ...
          >
            ...
            <span className="flex-1 min-w-0 text-sm font-bold text-gray-900">
              Other services
            </span>
            <ChevronRight className="h-4 w-4 text-gray-300" />
          </button>
          ...
          {isLoggedIn ? (
            <>
              <button
                type="button"
                onClick={() => goAuthRequired(getBikeRentDeleteAccountPath())}
              ...
              >
                ...
                <span className="min-w-0 flex-1 text-sm font-bold text-red-600">
                  Delete Account
                </span>

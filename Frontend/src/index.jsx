...
function resolveNativeInitialRoute() {
  ...
  const isCustomerModulePath = (path) =>
    ...
    path === '/bike-rent' ||
    path.startsWith('/bike-rent/')

  if (isCustomerModulePath(pathname)) return pathname
  ...
  if (isCustomerModulePath(storedRoute) || storedRoute.startsWith('/admin')) {
    return storedRoute
  }
  ...
  if (isModuleAuthenticated('user')) {
    try {
      ...
      if (isCustomerModulePath(remembered)) return remembered
    } catch {
      /* ignore */
    }
    return '/food/user'

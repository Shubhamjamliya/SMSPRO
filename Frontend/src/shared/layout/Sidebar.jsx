...
const colorMap = {
  ...
  orange:
    "text-primary bg-primary/5 border-primary/10 group-hover:bg-primary/10",
  green:
  ...
  red: "text-primary bg-primary/5 border-primary/10 group-hover:bg-primary/10",
  slate:
          ...
          <AnimatePresence>
            {isHovered && (
              <motion.div
                ...
                className={cn("absolute inset-0 rounded-lg -z-10", isSellerPanel ? "bg-primary/10" : isLightSidebar ? "bg-white/50" : "bg-white/5")}
                initial={{ opacity: 0 }}
                animate={{ opacity: 1 }}
                exit={{ opacity: 0 }}
                transition={{
          ...
          <div className="flex items-center space-x-2.5 z-10">
            <div
              className={cn(
                ...
                isChildActive || isOpen
                  ? (isSellerPanel ? "bg-primary text-white shadow-primary/40 ring-2 ring-primary/20" : "bg-primary text-white shadow-primary/40 ring-2 ring-primary/20")
                  : (isSellerPanel
          ...
          <div
            className={cn(
              ...
              isOpen
                ...
                : (isSellerPanel ? "rotate-0 text-primary/70 group-hover:text-primary" : isLightSidebar ? "rotate-0 text-slate-400 group-hover:text-slate-600" : "rotate-0 text-gray-600 group-hover:text-gray-400"),
            )}>
              ...
              <NavLink
                ...
                className={({ isActive }) =>
                  cn(
                    ...
                    isActive
                      ? (isSellerPanel
                          ? "text-primary font-bold bg-white/60 shadow-sm ring-1 ring-primary/20"
                          : isLightSidebar
                ...
                {({ isActive }) => (
                  <>
                    {isActive && (
                      <div className={cn("absolute left-0 top-1/2 -translate-y-1/2 w-0.5 h-3 rounded-full shadow-[0_0_10px_rgba(242,101,34,0.5)]", isSellerPanel ? "bg-primary" : "bg-primary")} />
                    )}
                    {child.label}
                  </>
                )}
    ...
    <NavLink
      ...
      className={({ isActive }) =>
        cn(
          ...
          isActive
            ? (isSellerPanel ? "bg-primary text-white shadow-primary/30" : "bg-primary text-white shadow-[0_10px_30px_rgba(var(--primary),0.3)]")
            : (isSellerPanel
                ? "text-slate-700 hover:text-primary hover:bg-primary/10"
                : isLightSidebar ? "text-[#5C5247] hover:text-[#1A1A1A]" : "text-gray-400 hover:text-white"),
        )
  ...
  return (
    <div className="flex flex-col h-full min-h-0">
      <div className={cn(
        ...
        isSellerPanel ? "border-primary/20 bg-primary/5" : isLightSidebar ? "border-slate-200 bg-white/10" : "border-white/5 bg-gradient-to-b from-white/[0.02] to-transparent"
      )}>
        <div className="flex flex-col justify-center">
          <div className="flex items-center space-x-2.5">
            {logoUrl ? (
              ...
              <div className={cn("h-8 w-8 rounded-xl flex items-center justify-center text-white shadow-lg transform -rotate-6 hover:rotate-0 transition-all duration-500 ease-out", isSellerPanel ? "bg-primary shadow-primary/30" : "bg-primary shadow-primary/30")}>
                <span className="text-lg font-black italic">{companyName?.charAt(0) || 'Z'}</span>
              </div>
            )}
            {!logoUrl && (
              <h1 className={cn("text-base font-black tracking-tight leading-none", isSellerPanel ? "text-slate-900" : isLightSidebar ? "text-[#1A1A1A]" : "text-white")}>
                {companyName || 'App'}
              </h1>
            )}
          </div>
          <span className={cn(
            ...
            isSellerPanel ? "bg-primary text-white" : isLightSidebar ? "bg-[#1A1A1A] text-white" : "bg-primary text-white"
          )}>
            {title}
          </span>
        </div>
        ...
        <button
          onClick={onClose}
          className={cn("p-2 md:hidden transition-colors", isSellerPanel ? "text-slate-800 hover:text-primary" : isLightSidebar ? "text-[#5C5247] hover:text-[#1A1A1A]" : "text-gray-500 hover:text-white")}
        >
          <X className="h-5 w-5" />
        </button>
      </div>
      ...
      >
        {isAdminPanel && (
          <div className="mb-4 px-1">
            ...
            <AdminModuleSwitcher variant="dark" />
          </div>
        )}
        <p className={cn("px-3 text-[9px] font-black uppercase tracking-[0.3em] mb-3", isSellerPanel ? "text-primary/70" : isLightSidebar ? "text-[#7C7062]" : "text-gray-600")}>
          Core Management
        </p>
        <AnimatePresence>
          {items.map((item, idx) => (
    ...
    <>
      ...
      <aside
        className={cn(
          ...
          isSellerPanel
            ? "bg-white backdrop-blur-md text-slate-900 border-primary/20"
            : isLightSidebar
            ...
            <div className="absolute left-0 inset-y-0 w-80 flex flex-col pointer-events-none">
              ...
              <motion.div
                ...
                className={cn(
                  ...
                  isSellerPanel
                    ? "bg-white backdrop-blur-md text-slate-900"
                    : isLightSidebar

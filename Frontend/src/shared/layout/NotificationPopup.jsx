...
const NotificationPopup = ({ notifications, onMarkAsRead, onMarkAllAsRead, onClose, isSeller, onNotificationClick }) => {
    return (
        <motion.div
        ...
        >
            <div className="p-4 border-b border-gray-50 flex items-center justify-between bg-slate-50/50 flex-shrink-0">
                <div className="flex items-center gap-2">
                    <div className={cn("h-8 w-8 rounded-lg flex items-center justify-center", isSeller ? "bg-primary/10 text-primary" : "bg-primary/10 text-primary")}>
                ...
                {notifications.length > 0 && (
                    <button
                        ...
                        className={cn("text-[10px] font-black uppercase tracking-widest transition-colors", isSeller ? "text-primary hover:text-primary/80" : "text-primary hover:text-primary/80")}
                    >
                        Mark all as read
                    </button>
                )}
                        ...
                        {notifications.map((notif) => (
                            <div
                                ...
                                className={cn(
                                    ...
                                    !notif.isRead && (isSeller ? "bg-primary/[0.02]" : "bg-primary/[0.02]")
                                )}
                                onClick={() => {
                            ...
                            >
                                {!notif.isRead && (
                                    <div className={cn("absolute left-0 top-0 bottom-0 w-1 rounded-r-full", isSeller ? "bg-primary" : "bg-primary")} />
                                )}
                                    ...
                                    <div className={cn(
                                        ...
                                        notif.type === 'order' ? "bg-emerald-50 text-emerald-600" :
                                            notif.type === 'payment' ? "bg-amber-50 text-amber-600" :
                                                (isSeller ? "bg-primary/5 text-primary" : "bg-blue-50 text-blue-600")
                                    )}>
